...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6192_Series_DP.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6192_SERIES_DP__
...
void ScalerDpAuxIntInitial(void)
{
    ...
    // Aux Clk Select Auto Enable, Aux New Mode, HW Fake Link Training Disable. 
    ...
    ScalerSetBit(P2_E2_AUX_PS_DUMMY1, ~(_BIT2 | _BIT1 | _BIT0), _BIT2);
...
{
    // Set DP Rate Manual
    ScalerSetBit(PB_AC_DP_FLD_08, ~_BIT5, _BIT5);
            
    // Enable Boost Stage 1,2,3, Boost 4 add DC Gain
    ScalerSetBit(PB_B1_DP_OOBS_SGN_DET, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT0), (_BIT7 | _BIT6 | _BIT5));

    // Select CDR 1pf cs 46pF
    ScalerSetBit(PB_A4_DP_FLD_00, ~(_BIT7 | _BIT6 | _BIT5), _BIT6);

    // Disable Loop Capacitor
    ScalerSetBit(PB_AA_DP_FLD_06, ~_BIT0, 0x00);
    ...
    ScalerSetBit(PB_D1_RX_BIST_01, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT2 | _BIT1 | _BIT0));
#else
    // Digital PHY Lane Swap Setting
...
{        
    if(ScalerGetBit_EXINT0(P2_E0_AUX_PS_CTRL, (_BIT4 | _BIT0)) == (_BIT4 | _BIT0))
    {
        if(ScalerGetBit_EXINT0(PC_F6_AUX_ERROR_HANDLE2, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00)
        {
            ...
            ScalerSetBit_EXINT0(PB_E2_DIG02, ~_BIT0, 0x00);        
...
void ScalerDpTp1SetPhy_EXINT0(void) using 1
{
    ...
    {        
        ...
        // CDR Setting
        ScalerSetByte_EXINT0(PB_A6_DP_FLD_02, 0x1A);
        
        // RP Rate Manual to 2.7G
        ScalerSetBit_EXINT0(PB_AC_DP_FLD_08, ~(_BIT7 | _BIT6), _BIT6);
    }
    else // RBR
    {        
        // CDR Setting
        ...
        // RP Rate Manual to 1.62G
        ScalerSetBit_EXINT0(PB_AC_DP_FLD_08, ~(_BIT7 | _BIT6), 0x00);
    }

    // Disable Vc Single Endded Mode
    ScalerSetBit_EXINT0(PB_B1_DP_OOBS_SGN_DET, ~(_BIT2 | _BIT1), _BIT1);

    // FLD Ref. Clock Select to D10.2
    ScalerSetBit_EXINT0(PB_AB_DP_FLD_07, ~_BIT6, 0x00);

    // For RBR
    ScalerSetBit_EXINT0(PB_C8_CMU_08, ~(_BIT7 | _BIT6 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT7 | _BIT2 | _BIT0));
    ScalerSetByte_EXINT0(PB_C9_TX, 0x0D);
    ScalerSetByte_EXINT0(PB_CA_SSCG, 0x86);

    // For HBR
    ScalerSetBit_EXINT0(PB_CB_TX_Z0_CALIBRATION, ~(_BIT7 | _BIT6), _BIT7);
    ScalerSetByte_EXINT0(PB_CC_DP_RESERVED_06, 0x0D);
    ScalerSetByte_EXINT0(PB_CD_DP_RESERVED_07, 0x86);
...
void ScalerDpTp2SetPhy_EXINT0(void) using 1
{
    ...
    if(ScalerDpGetDpcdInfo_EXINT0(0x00, 0x01, 0x00) == 0x0A) // HBR
    { 
        // Enable Vc Single Ended Mode
        ScalerSetBit_EXINT0(PB_B1_DP_OOBS_SGN_DET, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));        
    }

    // Reload DP FLD Band
    ScalerSetByte_EXINT0(PB_DC_TX_BIST_06, ScalerGetByte_EXINT0(PB_D8_TX_BIST_02));
    ScalerSetByte_EXINT0(PB_DD_TX_BIST_07, ScalerGetByte_EXINT0(PB_D9_TX_BIST_03));
    ScalerSetByte_EXINT0(PB_DE_TX_BIST_08, ScalerGetByte_EXINT0(PB_DA_TX_BIST_04));
    ScalerSetByte_EXINT0(PB_DF_TX_BIST_09, ScalerGetByte_EXINT0(PB_DB_TX_BIST_05));

    // FLD Manual Mode, Disable cp_manual, Disable Calib_manual
    ScalerSetBit_EXINT0(PB_AF_DP_RXMISC_02, ~(_BIT7 | _BIT5 | _BIT3), 0x00);
...
{            
    ...
    {        
        // VCO Setting
        ScalerSetByte(PB_A5_DP_FLD_01, 0x31);

        // CDR Setting
        ScalerSetByte(PB_A6_DP_FLD_02, 0x1A);

        // RP Rate Manual to 2.7G
        ScalerSetBit(PB_AC_DP_FLD_08, ~(_BIT7 | _BIT6), _BIT6);

        // Enable Vc Single Ended Mode
        ScalerSetBit(PB_B1_DP_OOBS_SGN_DET, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));  
    }
    else // RBR
    {        
        // VCO Setting
        ScalerSetByte(PB_A5_DP_FLD_01, 0x71);

        // CDR Setting
        ScalerSetByte(PB_A6_DP_FLD_02, 0x10);

        // RP Rate Manual to 1.62G
        ScalerSetBit(PB_AC_DP_FLD_08, ~(_BIT7 | _BIT6), 0x00);

        // Disable Vc Single Endded Mode
        ScalerSetBit(PB_B1_DP_OOBS_SGN_DET, ~(_BIT2 | _BIT1), _BIT1);
    }

    // FLD Ref. Clock Select to GDI Clock
    ScalerSetBit(PB_AB_DP_FLD_07, ~_BIT6, _BIT6);

    // For RBR
    ScalerSetBit(PB_C8_CMU_08, ~(_BIT7 | _BIT6), _BIT7);
    ScalerSetByte(PB_C9_TX, 0x2B);
    ScalerSetByte(PB_CA_SSCG, 0x31);
       
    // For HBR
    ScalerSetBit(PB_CB_TX_Z0_CALIBRATION, ~(_BIT7 | _BIT6), _BIT7);
    ScalerSetByte(PB_CC_DP_RESERVED_06, 0x2B);
    ScalerSetByte(PB_CD_DP_RESERVED_07, 0x1D);

    // FLD Auto Mode
    ScalerSetBit(PB_AF_DP_RXMISC_02, ~_BIT7, _BIT7);       

    // Enable Adaptive EQ
    ScalerSetBit(PB_AD_DP_FLD_09, ~_BIT3, 0x00);
    
    // Enable De-Scrambling, Enable Commoa Detection
...
void ScalerDpPhyCtsTp1SetPhy(void)
{
    ...
    // FLD Ref. Clock Select to D10.2
    ScalerSetBit(PB_AB_DP_FLD_07, ~_BIT6, 0x00);

    // For RBR
    ScalerSetBit(PB_C8_CMU_08, ~(_BIT7 | _BIT6 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT7 | _BIT2 | _BIT0));
    ScalerSetByte(PB_C9_TX, 0x0D);
    ScalerSetByte(PB_CA_SSCG, 0x86);

    // For HBR
    ScalerSetBit(PB_CB_TX_Z0_CALIBRATION, ~(_BIT7 | _BIT6), _BIT7);
    ScalerSetByte(PB_CC_DP_RESERVED_06, 0x0D);
    ScalerSetByte(PB_CD_DP_RESERVED_07, 0x86);

    // FLD Auto Mode
    ScalerSetBit(PB_AF_DP_RXMISC_02, ~_BIT7, _BIT7);       

    // Disable Adaptive EQ
    ScalerSetBit(PB_AD_DP_FLD_09, ~_BIT3, _BIT3);
...
void ScalerDpPhyCtsTp2SetPhy(void)
{
    ...
    { 
        // Enable Vc Single Ended Mode
        ScalerSetBit(PB_B1_DP_OOBS_SGN_DET, ~(_BIT2 | _BIT1), _BIT2);        
    }

    // Reload DP FLD Band
    ScalerSetByte(PB_DC_TX_BIST_06, ScalerGetByte(PB_D8_TX_BIST_02));
    ScalerSetByte(PB_DD_TX_BIST_07, ScalerGetByte(PB_D9_TX_BIST_03));
    ScalerSetByte(PB_DE_TX_BIST_08, ScalerGetByte(PB_DA_TX_BIST_04));
    ScalerSetByte(PB_DF_TX_BIST_09, ScalerGetByte(PB_DB_TX_BIST_05));

    // FLD Manual Mode, Disable cp_manual, Disable Calib_manual
    ScalerSetBit(PB_AF_DP_RXMISC_02, ~(_BIT7 | _BIT5 | _BIT3), 0x00);
    ...
    while(GET_DP_PHY_CTS_FLG() == _TRUE)
    {
        ...
        {    
            ...
            case 0x03:
                ...
                ScalerSetDataPortByte(PC_B3_DP_ACCESS_PORT, 0xD4, 0x00); 
...
void ScalerDpASSRModeSetting(void)
{
    if((ScalerDpGetDpcdInfo(0x00, 0x01, 0x0A) & _BIT0) == _BIT0)
    {
        ...
        ScalerSetBit_EXINT0(PB_EE_RESERVED_A, ~_BIT0, _BIT0);
    }
    else
    {
        // Scramble seed equal to 0xFFFF
        ScalerSetBit_EXINT0(PB_EE_RESERVED_A, ~_BIT0, 0x00);
    }
}
