...
#define __RL6269_SERIES_SDRAM__
...
void ScalerSDRAMPowerControl(bit bOn)
{
    if(bOn == _ON)
    {
        ...
        ScalerSetBit(P10_F3_PIN_SDRAM_CTRL53, ~(_BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT0));
#elif(_SDRAM_SIZE_CONFIG == _2M_32BIT)
    ...
    else
    {
        ...
        // Pin share MCM SDRAM
        ScalerSetBit(P10_F3_PIN_SDRAM_CTRL53, ~_BIT2, _BIT2); 
        ...
        // Pin share no SDRAM
        ScalerSetBit(P10_F3_PIN_SDRAM_CTRL53, ~_BIT2, 0x00);
...
void ScalerSDRAMInitial(void)
{
    ...
    ScalerSetByte(P4_C1_SDR_CLK_DLY2, _SDR_CLK_DLY2);
    ScalerSetByte(P4_C3_DQS0_DLY2, _SDR_DQS_DLY);
    ScalerSetByte(P4_C5_DQS1_DLY2, 0x14);
...
void ScalerSDRAMSetODThreeBlockAddress(void)
{
    ...
    {
        // Get FIFO input size
        ScalerGetDataPortByte(CM_30_FIFO_ACCESS_PORT, _CM_31_PT_00_FIFO_DRL_H_BSU, 1, &pData[0], _NON_AUTOINC);
        ScalerGetDataPortByte(CM_30_FIFO_ACCESS_PORT, _CM_31_PT_01_FIFO_DRW_L_BSU, 1, &pData[1], _NON_AUTOINC);
        ScalerGetDataPortByte(CM_30_FIFO_ACCESS_PORT, _CM_31_PT_02_FIFO_DRL_L_BSU, 1, &pData[2], _NON_AUTOINC);

        // Get FIFO input H Width
        PDATA_WORD(3) = ((pData[0] & 0xF0) << 4) | pData[1];

        // Get FIFO input V Height
        PDATA_WORD(4) = ((pData[0] & 0x0F) << 8) | pData[2];        
...
#endif  // End of #if((_OD_SUPPORT == _ON) || (_SDRAM_PHASE_CALIBRATION == _ON))
...
void ScalerSDRAMSetHWAutoBlockToggle(void)
{
    ...
    ScalerSetByte(P5_EB_SDRF_MN_MADDR_TOG_REM, (pData[1] & 0xFF));    
}
        ...
        for(ucSdrDqsFineDly = 0; ucSdrDqsFineDly <= ucSdrDqsFineDlyMax; ucSdrDqsFineDly++)
        {
            ...
            if(bIndex == _TRUE)
            {
                ...
                if((bReadPhaseFlag_last == _TRUE) && (bReadPhaseFlag == _FALSE) && (bGoodStartFlag == _TRUE))
                {
                    ...
                    if(ucStart <= 1)
                    {
                        g_ucConflictArea = ucSdrDqsFineDly + 1;
                    }
