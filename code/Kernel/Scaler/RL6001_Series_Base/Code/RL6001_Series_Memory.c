...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6001_Series_Memory.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6001_SERIES_MEMORY__
...
{    
    ...
    ScalerSetBit(CM_35_FILTER_CTRL, ~(_BIT7 | _BIT5 | _BIT4), _BIT4);    
}
...
void ScalerMemoryFrameSyncSetDt(void)
{
    ...
    if(ScalerGetBit(CM_28_VDISP_CTRL, _BIT3) == _BIT3)
    {
        ...
        PDATA_DWORD(0) = (WORD)ScalerGetByte(CM_40_IVS2DVS_DEALY_LINES) + ScalerVgipGetCaptureVDelay();    
