...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6373_Series_AutoFunc.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6373_SERIES_AUTOFUNC__
...
#define GET_ADC_SHA_GAIN()                       ((ScalerGetByte(P0_CE_ADC_CTL_RGB) & 0xE0) >> 5)
...
void ScalerAutoInitializeAutoColor(EnumAutoColorType enumAutoType)
{
    ...
    // ABL old mode; ABL offset adjust old mode
    ScalerSetBit(P0_EC_ABL_R_NOISE_VALUE, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT2), 0x00);
...
{   
    if(ucShaFrom != ucShaTo)
    {
        if(ucShaTo > 4)
        {
            // 5: 2126; 6: 2314; 7: 2524
            PDATA_WORD(0) = ((ucShaTo == 5) ? 2126 : ((ucShaTo == 6) ? 2314 : 2524));
        }
        else
        {
            //0: 1574, step = 100
            PDATA_WORD(0) = 1574 + ucShaTo * 100;
        }

        if(ucShaFrom > 4)
        {
            // 5: 2126; 6: 2314; 7: 2524
            PDATA_WORD(1) = ((ucShaFrom == 5) ? 2126 : ((ucShaFrom == 6) ? 2314 : 2524));
        }
        else
        {
            //0: 1574, step = 100
            PDATA_WORD(1) = 1574 + ucShaFrom * 100;
        }
        
        if(enumType == _ADC_GAIN)
        {
            ucValue = (BYTE)((((DWORD)ucValue + 51) * PDATA_WORD(0) / PDATA_WORD(1)) - 51); // Gain
        }
        else
        {
...
{    
    ...
    {
        ucResult = (BYTE)(((DWORD)(ucTarget - ucCurrent) * ((WORD)ucGain + 51)) / ucTarget);
    }
...
EnumAutoResult ScalerAutoMainToneMeasure(BYTE ucStartLevel, BYTE *ucMainTone)
{
    ...
    *ucMainTone = 3;    
    ...
    // HW auto for 5 steps; Step size = 0;
    ScalerSetByte(CM_7B_HW_AUTO_PHASE_CTRL0, 0x27);
...
StructAdcDataType ScalerAutoBalanceYPbPrFromFormula(StructAdcDataType * stAdcData)
{
    ...
    stYPbPrData.ucAdcGainMSB[0] += (BYTE)(((DWORD)stYPbPrData.ucAdcGainMSB[0] + 51) * 31 / 224) + _AUTO_PR_GAIN_BIAS; // Pr
    stYPbPrData.ucAdcGainMSB[2] += (BYTE)(((DWORD)stYPbPrData.ucAdcGainMSB[2] + 51) * 31 / 224) + _AUTO_PB_GAIN_BIAS; // Pb
    
    stYPbPrData.ucAdcOffsetMSB[1] = stYPbPrData.ucAdcOffsetMSB[1] + _AUTO_Y_OFFSET_BIAS;// Y
    stYPbPrData.ucAdcGainMSB[1] += (BYTE)(((DWORD)stYPbPrData.ucAdcGainMSB[1] + 51) * 36 / 219) + _AUTO_Y_GAIN_BIAS; // Y
...
EnumAutoResult ScalerAutoAblMeasure(void)
{
    ...
    ScalerSetBit(P0_EC_ABL_R_NOISE_VALUE, ~(_BIT3 | _BIT2), (_BIT3 | _BIT2));
    ...
    ScalerSetBit(P0_EC_ABL_R_NOISE_VALUE, ~(_BIT3 | _BIT2), 0x00);    
