...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6336_Series_ADC.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6336_SERIES_ADC__
...
#warning "Not Apply the Solution in RL6336 Series!!"
#endif
...
#if(_DIGITAL_PORT_SUPPORT == _OFF)
        // Enable GDI bandgap for DPLL
...
void ScalerADCOutputClkSetting(void)
{
    ...
    // Output clock divider Polarity Normal, Duty Staiblizer is 49.3%
    ScalerSetBit(P0_D9_TEST_PTN_VPOS_L, ~_BIT2, 0x00);
...
void ScalerADCBiasCurrentSetting(void)
{
    ...
    // Set ib_vb 40uA
    ScalerSetBit(P0_DB_TEST_PTN_RD, ~_BIT3, 0x00); 
...
{
    // Set ADC PGA
    ScalerSetBit(P0_CE_ADC_CTL_RGB, ~(_BIT7 | _BIT6 | _BIT5), (ucAdcPGA << 5) & 0xE0); 
}
...
void ScalerADCPGAbyPassSetting(void)
{
    // 6336 have no PGA by pass option
}
...
#if((_A0_HS_SCHMITT_TRIGGER_POLARITY_SEL == _NEGATIVE) && \
    ((_A0_HS_SCHMITT_TRIGGER_POSITIVE_VTH >= _HS_SCHMITT_TRIGGER_LEVEL_10) || (_A0_HS_SCHMITT_TRIGGER_NEGATIVE_VTH >= _HS_SCHMITT_TRIGGER_LEVEL_10)))
