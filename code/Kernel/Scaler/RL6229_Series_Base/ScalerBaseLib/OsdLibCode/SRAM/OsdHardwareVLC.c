...
//****************************************************************************
void ScalerOsdHardwareVLC(BYTE *pArray, WORD usVLCTableSize, BYTE ucBankNum, WORD usOffset, WORD usFontTableStart, EnumOsdRotateType enumOsdRotateType);
...
{
    BYTE ucVlcType = 0;
    ucVlcType = ScalerOsdFrameControlByteGet(0x003, _OSD_BYTE2) & (_BIT1);

    if(enumOsdRotateType == _OSD_ROTATE_DEGREE_90)
    {
        ucVlcType = ucVlcType | _OSD_ROTATE_CW | _OSD_ROTATE_HARDWARE_ROTATION_ENABLE | _OSD_ROTATE_ROTATION;
    }
    else if(enumOsdRotateType == _OSD_ROTATE_DEGREE_270)
    {
        ucVlcType = ucVlcType | _OSD_ROTATE_CCW | _OSD_ROTATE_HARDWARE_ROTATION_ENABLE | _OSD_ROTATE_ROTATION;
    }
    else
    {
        ucVlcType = ucVlcType | _OSD_ROTATE_NO_SWAP;
    }

    // rotation
    ScalerOsdFrameControlByte(0x003, _OSD_BYTE2, ucVlcType);
    ...
    ScalerOsdBurstWriteDataPort(pArray, usVLCTableSize, ucBankNum, _BURSTWRITE_DATA_OSD, _BURSTWRITE_FROM_FLASH);
