...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6336_Series_Scaling.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6336_SERIES_SCALING__
...
void ScalerScalingSetScalingUp(StructSDRAMDataInfo stFIFOScaling)
{
    ...
    if(stFIFOScaling.usOutputVHeight < g_stMDomainOutputData.usVHeight)
    {
        ...
        {
            ScalerSetDataPortByte(CM_33_SU_ACCESS_PORT, _CM_34_PT_11_SU_V_COEF_INI_VALUE, 0x80);        
        }
    }
    else
    {
    ...
    if(stFIFOScaling.usOutputHWidth < g_stMDomainOutputData.usHWidth)
    {
        ...
        // ideal initial phase = (1+R)*16*8 - 0x80        
        PDATA_DWORD(0) = (DWORD)(stFIFOScaling.usOutputHWidth + g_stMDomainOutputData.usHWidth) * 16 * 8 / g_stMDomainOutputData.usHWidth - 0x80;
    ...
    else
    {
        ...
        ScalerSetDataPortByte(CM_33_SU_ACCESS_PORT, _CM_34_PT_10_SU_H_COEF_INI_VALUE, 0x7F);
    }  
