...
bit UserAdjustCheckAspectRatio(void)
{	
...
#define CONTRAST_RATIO_HI       (((WORD)ucContrast-_OSD_CONTRAST_DEF_VALUE)*(MAX_CONTRAST_RATIO-DEF_CONTRAST_RATIO)/(100-_OSD_CONTRAST_DEF_VALUE)+DEF_CONTRAST_RATIO)
#define CONTRAST_RATIO_LO       ((WORD)ucContrast*(DEF_CONTRAST_RATIO-MIN_CONTRAST_RATIO)/(_OSD_CONTRAST_DEF_VALUE)+MIN_CONTRAST_RATIO)
#define COLOR_RANGE_0_100       28    // 100 is 1:1 value
#define COLOR_RANGE_0_50_100    78    // 50 is 1:1 value
#define COLOR_RANGE             COLOR_RANGE_0_100

void UserAdjustContrast(BYTE ucContrast)
{
    if(ucContrast>= _OSD_CONTRAST_DEF_VALUE)
    {
        pData[0] = (((WORD)g_stColorProcData.ucColorTempR+COLOR_RANGE)*CONTRAST_RATIO_HI/100);
        pData[1] = (((WORD)g_stColorProcData.ucColorTempG+COLOR_RANGE)*CONTRAST_RATIO_HI/100);
        pData[2] = (((WORD)g_stColorProcData.ucColorTempB+COLOR_RANGE)*CONTRAST_RATIO_HI/100);
    }
    else
    {
        pData[0] = (((WORD)g_stColorProcData.ucColorTempR+COLOR_RANGE)*CONTRAST_RATIO_LO/100);
        pData[1] = (((WORD)g_stColorProcData.ucColorTempG+COLOR_RANGE)*CONTRAST_RATIO_LO/100);
        pData[2] = (((WORD)g_stColorProcData.ucColorTempB+COLOR_RANGE)*CONTRAST_RATIO_LO/100);
    }
    if(pData[0]>255)
    {
       pData[0]=255;
...
//--------------------------------------------------
// Description  : UserAdjustBacklightPercent
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void UserAdjustBacklightPercent(BYTE ucValue)
...
void UserAdjustSharpness(BYTE ucSharpness)
{
    //Eli 20121213 Modify FW coding.
  //  if((GET_INPUT_TIMING_HWIDTH() == _PANEL_DH_WIDTH) && (GET_INPUT_TIMING_VHEIGHT() == _PANEL_DV_HEIGHT))
  //  {
  //      ScalerMemorySetScalingCoef(tSCALE_COEF_TABLE_1_TO_1[ucSharpness], tSCALE_COEF_TABLE_1_TO_1[ucSharpness], _SHARPNESS_TABLE_SIZE, GET_CURRENT_BANK_NUMBER());
  //  }
#if(_3D_SUPPORT == _ON)
...
#endif        
		//ScalerColorDCROff();//Robert Wang 20130206 update for STD code
...
void UserAdjustDCRPwm(BYTE ucPwmValue)
{
	...
	else
	{
		...
		{
			g_ucPreDCRPWM -= 1;
            //ScalerDebugMessage("g_ucPreDCRPWM--", g_ucPreDCRPWM);    
		}
		...
		if(g_bDCRBLOffStart==_ON)
		{
            ...
            //ScalerDebugMessage("@@@Backlight OFF !!", ucPwmValue);            
		}
		else
#endif
		{		 
	...
	//DebugMessageOsd("DCR PWM target", ucPwmValue);
	//DebugMessageOsd("DCR PWM current", g_ucPreDCRPWM);
}
...
void UserAdjustDCRMeasure(void)
{
	...
	// Get total sum (R+G+B) of  pixel value above thershold1 value三槍量測值
	ucValueAvg =((DWORD) ScalerColorDCRReadResult(_DCR_ABOVE_TH1_VAL) / GET_MDOMAIN_OUTPUT_HWIDTH() / GET_MDOMAIN_OUTPUT_VHEIGHT());    
	//DebugMessageOsd("DCR Measure result", ucValueAvg);
...
//            ucRGain = ScalerColorDCRReadResult(_HIGH_LV_VAL_R);
			...
			// dynamic detect the hightest level value of R/G/B chanel
...
#endif// #if(_USER_DCR_FUNCTION == _ON)
