...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6192_Series_AutoFunc.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6192_SERIES_AUTOFUNC__
...
void ScalerAutoInitializeAutoColor(EnumAutoColorType enumAutoType)
{
    ...
    // Max frame/field count = 7; ABL width x1;
...
EnumAutoResult ScalerAutoUpdateAdcGainCalibrationRGB(BYTE ucGainBias)
{
    ...
    for(ucIndex = 0; ucIndex < 3; ++ucIndex)
    {
        ...
        // Real swing = (max - 0x80) * 2
        pData[0] = ScalerGetByte(P0_E9_ABL_R_VALUE + ucIndex);                
        pData[0] = ((pData[0] - 0x80) << 1) + 1;

        // Update ADC gain
        if(pData[0] < ucGainBias)
        {
            pData[0] = ScalerAutoConvertAdcGainStep(ucGain, ucGainBias, pData[0]);
            ucGain -= (pData[0] == 0) ? 1 : pData[0];
        }
        else
        {
            ucGain += (pData[0] - ucGainBias);
        }
...
{ 
    ...
    // Check for original signal level
...
void ScalerAutoInitializeCalibration(void)
{
    // Initial VGA data pathd
    ScalerSyncVgaScanInitial();
    ScalerADCClampMode(_COLOR_SPACE_RGB);

    // Enable calibration voltage; Always clamp; Volage 1.1V;
    ScalerSetByte(P0_D2_ADC_SOG_CMP, (_BIT7 | _BIT5 | _BIT2 | _BIT1 | _BIT0));    
...
EnumAutoResult ScalerAutoAblMeasure(void)
{
    ...
    // Enable calibration voltage
    ScalerSetBit(P0_D2_ADC_SOG_CMP, ~_BIT4, _BIT4);
    ...
    // Disable calibration voltage
    ScalerSetBit(P0_D2_ADC_SOG_CMP, ~_BIT4, 0x00);
