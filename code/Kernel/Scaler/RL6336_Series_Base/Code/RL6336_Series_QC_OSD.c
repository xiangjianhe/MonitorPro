...
#define __RL6336_SERIES_QC_OSD__
...
void ScalerQCOsdRegInitial(void)
{
    ...
    ScalerOsdFrameControlAllByte(0x004, _OSD_WITHOUT_DB, 0x00, 0x00, 0x00);
    ScalerOsdFrameControlAllByte(0x005, _OSD_WITHOUT_DB, 0x00, 0x00, 0x00);
    ScalerOsdFrameControlAllByte(0x006, _OSD_WITHOUT_DB, 0x00, 0x00, 0x00);
    ScalerOsdFrameControlByte(0x007, _OSD_BYTE0, 0x00);
    ScalerOsdFrameControlByte(0x007, _OSD_BYTE1, 0x00);
    ScalerOsdFrameControlAllByte(0x012, _OSD_WITHOUT_DB, 0x00, 0x00, 0x00);
...
void ScalerQCOsdWindowGridLine(bit bENABLE, BYTE ucStatus)
{
    ...
    ScalerQCOsdSetGridLine(_OSD_WINDOW_2, (0 + (ucStatus * 3)), 61, _ENABLE, _ENABLE, 0x03, 0x2710, _DISABLE);
    ...
    ScalerQCOsdSetGridLine(_OSD_WINDOW_3, 1, 62, _DISABLE, _ENABLE, 0x03, 0x2710, _DISABLE);
...
//--------------------------------------------------
// Description  : Test window 0, 5, 6, 8, 9 gradient function
...
{        
    ...
    //vertical direction
    ScalerQCOsdDrawWindow(bEnable, _OSD_WINDOW_7, 44, X_START(_OSD_WINDOW_7_X_START), Y_START(_OSD_WINDOW_7_Y_START), X_END(_OSD_WINDOW_7_X_START + _OSD_WINDOW_WIDTH), Y_END(_OSD_WINDOW_7_Y_START + _OSD_WINDOW_HEIGHT));
...
bit ScalerQCOsdTestProc(void)
{
    ...
    if(ScalerQCCheckCRCOn(0x38, 0x81, 0x5A, _D_DOMAIN_CRC))
   ...
   if(ScalerQCCheckCRCOn(0x11, 0xC5, 0xC7, _D_DOMAIN_CRC))
    {
        usData |= _BIT2;
    }
    ...
    if(ScalerQCCheckCRCOn(0x7F, 0x6B, 0xD6, _D_DOMAIN_CRC))
    {
        usData |= _BIT3;
    }    
    ...
    if(ScalerQCCheckCRCOn(0x6F, 0x8B, 0x60, _D_DOMAIN_CRC))
    {
        usData |= _BIT4;
    }    
    ...
    if(ScalerQCCheckCRCOn(0x79, 0x29, 0x3C, _D_DOMAIN_CRC))
    {
        usData |= _BIT5;
    }
    ...
    if(ScalerQCCheckCRCOn(0x0E, 0xA3, 0x68, _D_DOMAIN_CRC))
    {
        usData |= _BIT6;
    }
    ...
    if(ScalerQCCheckCRCOn(0x46, 0x3F, 0x3E, _D_DOMAIN_CRC))
    {
        usData |= _BIT7;
    }    
    ...
    if(ScalerQCCheckCRCOn(0x55, 0x8C, 0xF5, _D_DOMAIN_CRC))
    {
        usData |= _BIT1;
    }    
