...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6229_Series_PLL.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6229_SERIES_PLL__
...
//--------------------------------------------------
#define _LVDS_PLL_BOUNDRY_0                         (250000 * (1 << _DISP_OUTPUT_PORT) / 7)
#define _LVDS_PLL_BOUNDRY_1                         (320000 * (1 << _DISP_OUTPUT_PORT) / 7)
#define _LVDS_PLL_BOUNDRY_2                         (380000 * (1 << _DISP_OUTPUT_PORT) / 7)
...
void ScalerPllSetDpllFreq(DWORD ulFreq)
{
    ...
    else
    {
        ...
        // pData[15]: [7:6]=LVDS_PLL_VCO  
        if(ulFreq  < _LVDS_PLL_BOUNDRY_0)
        {
            pData[15] |= (0 << 6);
        }
        else if(ulFreq < _LVDS_PLL_BOUNDRY_1)
        {
            pData[15] |= (1 << 6);
        }   
        else if(ulFreq < _LVDS_PLL_BOUNDRY_2)
        {
            pData[15] |= (2 << 6);
        } 
        else
    ...
    switch(ucDPLLSettingState)
    {
        ...
        case _DPLL_SET_VCO_BAND:
            ...
            // Manual set DPLL VCO band
            ScalerSetBit(P1_C3_DPLL_CAL, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), ((pData[15] & 0x03) << 6) | _BIT5);

            // Manual set LVDS VCO band
            ScalerSetDataPortBit(CM_8B_TCON_ADDR_PORT, _CM_8C_PT_12_LVDS_CTRL33, ~(_BIT7 | _BIT6 | _BIT5), ((pData[15] & 0xC0) | _BIT5));
