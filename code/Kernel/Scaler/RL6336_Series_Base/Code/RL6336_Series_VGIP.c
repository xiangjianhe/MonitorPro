...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6336_Series_VGIP.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6336_SERIES_VGIP__
...
void ScalerVgipInputInitial(BYTE ucSourceType)
{
    ...
    // Reset field detect edge; Reset VGIP clock polarity
    ScalerSetBit(CM_11_VGIP_SIGINV, ~(_BIT6 | _BIT5 | _BIT3 | _BIT0), 0x00);
...
void ScalerVgipSetCaptureHDelay(WORD usIHSDelay)
{
    ...
    ScalerSetBit(CM_1E_VGIP_HV_DELAY, ~_BIT2, ((HIBYTE(usIHSDelay) << 1) & _BIT2));
...
WORD ScalerVgipGetCaptureHDelay(void)
{
    return ((((WORD)ScalerGetByte(CM_1E_VGIP_HV_DELAY) & _BIT2) << 7) | (((WORD)ScalerGetByte(CM_1E_VGIP_HV_DELAY) & _BIT0) << 8) | ScalerGetByte(CM_1D_IHS_DELAY));    
}
