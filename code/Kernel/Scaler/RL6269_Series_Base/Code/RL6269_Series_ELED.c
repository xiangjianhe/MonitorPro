...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6269_Series_ELED.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6269_SERIES_ELED__
...
void ScalerELEDInitial(void)
{
    ...
    ScalerSetBit(P16_C4_LEDBC_ADVREF4, ~_BIT7, _BIT7);

    // Set LED_EN Select New Mode
    ScalerSetBit(P16_C4_LEDBC_ADVREF4, ~_BIT3, _BIT3);

    if(ScalerGetByte(CM_0F_ID_REG_02) >= 0x54)
    {
...
#else // Else of #if(_ELED_VFB_RESISTOR == _VFB_RESISTOR_10_OHM)
...
#endif // End of #if(_ELED_VFB_RESISTOR == _VFB_RESISTOR_10_OHM)

    // Set Total Current
    ScalerSetBit(P16_C8_LEDBC_ADVREF0, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), PDATA_WORD(0));

    ScalerRead(P24_A7_EFUSE_DATA1, 7, pData, _AUTOINC);

    if((pData[0] == 0x00) && (pData[1] == 0x00) && (pData[2] == 0x00) && (pData[3] == 0x00) && 
       (pData[4] == 0x00) && (pData[5] == 0x00) && (pData[6] == 0x00))
    {

// E-fuse Resistor by each IC different
#if(_ELED_VFB_RESISTOR == _VFB_RESISTOR_5_OHM)

        ScalerSetByte(P24_A6_EFUSE_DATA0, 0x60);
        ScalerSetByte(P24_A7_EFUSE_DATA1, 0x34);
        ScalerSetByte(P24_A8_EFUSE_DATA2, 0x1A);
        ScalerSetByte(P24_A9_EFUSE_DATA3, 0x8D);
        ScalerSetByte(P24_AA_EFUSE_DATA4, 0x46);
        ScalerSetByte(P24_AB_EFUSE_DATA5, 0xA3);
        ScalerSetByte(P24_AC_EFUSE_DATA6, 0xD1);
        ScalerSetByte(P24_AD_EFUSE_DATA7, 0x48);
    ...
    ScalerSetByte(P16_B7_LEDBC_VMINSDISB, ~((_ELED_STRING0) | (_ELED_STRING0 << 1) | (_ELED_STRING1 << 2) | (_ELED_STRING1 << 3) | (_ELED_STRING2 << 4) | (_ELED_STRING2 << 5) | (_ELED_STRING3 << 6) | (_ELED_STRING3 << 7)));
}
...
void ScalerELEDDimmingClk(void)
{
    ...
    ScalerSetByte(P16_DD_LEDBC_LED_PWM_CTRL, 0xB0);
    ...
    ScalerSetByte(P16_E2_LEDBC_LED_PWM_EN, ((_ELED_STRING0) | (_ELED_STRING0 << 1) | (_ELED_STRING1 << 2) | (_ELED_STRING1 << 3) | (_ELED_STRING2 << 4) | (_ELED_STRING2 << 5) | (_ELED_STRING3 << 6) | (_ELED_STRING3 << 7)));
    ...
    if(enumBoosterStatus == _ELED_BOOSTING_STATE)
    {
        ...
        if(ScalerGetByte(CM_0F_ID_REG_02) >= 0x54)
        {
            ...
            for(ucOvpCount = 30; ucOvpCount <= 51; ucOvpCount += 3)
            {
                if(((WORD)_ELED_PCB_OVP_RATION * ucOvpCount) >= _ELED_PCB_OVP_TRIGGER_LEVEL)
                {
                    ScalerSetBit(P16_C5_LEDBC_ADVREF3, ~(_BIT3 | _BIT2 | _BIT1), ucOvpParameter << 1);
                    break;
                }
            ...
            {
                ScalerSetBit(P16_C5_LEDBC_ADVREF3, ~(_BIT3 | _BIT2 | _BIT1), (_BIT3 | _BIT2 | _BIT1));
            }
#endif
        }
        else
