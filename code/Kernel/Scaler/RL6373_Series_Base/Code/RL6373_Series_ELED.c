...
void ScalerELEDInitial(void)
{
	...
	ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~(_BIT3 | _BIT2), _BIT3);

#elif(_PANEL_EMBEDDED_LED_DRIVER_DIMMING_SEL == _ANALOG_DIMMING)

#if(_ELED_VREF_INITIAL_CURRENT == _VREF_INITIAL_1P5_UA)
    ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~(_BIT3 | _BIT2), (_BIT3 | _BIT2));

#elif(_ELED_VREF_INITIAL_CURRENT == _VREF_INITIAL_1_UA)
    ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~(_BIT3 | _BIT2), _BIT3);

#elif(_ELED_VREF_INITIAL_CURRENT == _VREF_INITIAL_0P5_UA)
    ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~(_BIT3 | _BIT2), 0x00);

#endif // End of #if(_ELED_VREF_INITIAL_CURRENT == _VREF_INITIAL_1P5_UA)
...
void ScalerELEDBacklightAdjust(BYTE ucBacklight)
{
    ...
    PDATA_WORD(1) = (((DWORD)PDATA_WORD(0) * (ucBacklight + 1)) / 255);
    PDATA_WORD(3) = ((ScalerGetByte(P16_C8_LEDBC_ADVREF0) << 1) | (ScalerGetBit(P16_CB_LEDBC_CODN7_6, _BIT7) >> 7));
    pData[4] = 4;

    // When Analog Dimming Code > 4 Fix Booster Glitch Disable
    if(abs(PDATA_WORD(1) - PDATA_WORD(3)) >= pData[4])
    {
        ScalerSetBit(P16_C5_LEDBC_ADVREF3, ~(_BIT3 | _BIT2), 0x00);        
    }

    // When Analog Dimming Code > 4 For Adjust Max Code 4 Step By Step
    for(pData[5] = 0; pData[5] < (512 / pData[4]); pData[5] += 1)
    {
        if(abs(PDATA_WORD(1) - PDATA_WORD(3)) >= pData[4])
        {
            if(PDATA_WORD(1) >= PDATA_WORD(3))
            {
                PDATA_WORD(3) += pData[4];
            }
            else
            {
                PDATA_WORD(3) -= pData[4];
            ...
            ScalerSetBit(P16_C8_LEDBC_ADVREF0, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (PDATA_WORD(3) >> 1));
            ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT7 , (PDATA_WORD(3) << 7));
        }
        else
        {
            ScalerSetBit(P16_C8_LEDBC_ADVREF0, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (PDATA_WORD(1) >> 1));
            ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT7 , (PDATA_WORD(1) << 7));
            break;
        }
    }

    // When Backlight < 90% Disable All eLED Short Fucntion & Fix Booster Glitch Function
    if(PDATA_WORD(1) < ((PDATA_WORD(0) * 9) / 10))
    {
        // Fix Booster Glitch Disable
        ScalerSetBit(P16_C5_LEDBC_ADVREF3, ~(_BIT3 | _BIT2), 0x00);     
        
        // All eLED Function Disable
        ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT4, 0x00);        
    }
    else
    {
        // Fix Booster Glitch Enable
        ...
        // All eLED Function Enable
        ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT4, _BIT4);         
    }
...
#if(_PANEL_EMBEDDED_LED_DRIVER_DIMMING_SEL == _DIGITAL_DIMMING)
    PDATA_WORD(0) = ((((WORD)_PANEL_ELED_STRING_NUMBER * _PANEL_ELED_STRING_CURRENT * _ELED_VFB_RESISTOR) / _ELED_PCB_CURRENT_CHANNEL) * 10 - _ELED_LED_DRIVER_MIN_PWM_DUTY) / _ELED_VREF_INITIAL_CURRENT;
#else
    PDATA_WORD(0) = ((((WORD)_PANEL_ELED_STRING_NUMBER * _PANEL_ELED_STRING_CURRENT * _ELED_VFB_RESISTOR) / _ELED_PCB_CURRENT_CHANNEL) * 10 - _ELED_LED_DRIVER_MIN_VOLTAGE) / _ELED_VREF_INITIAL_CURRENT;
#endif
    
    // Set Total Current
    ScalerSetBit(P16_C8_LEDBC_ADVREF0, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (PDATA_WORD(0) >> 1));
    ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT7 , (PDATA_WORD(0) << 8));
...
// E-fuse Resistor by each IC different
        ...
        ScalerSetByte(P24_A6_EFUSE_DATA0, 0x28);
        ScalerSetByte(P24_A7_EFUSE_DATA1, 0x28);
        ScalerSetByte(P24_A8_EFUSE_DATA2, 0x28);
        ScalerSetByte(P24_A9_EFUSE_DATA3, 0x28);
...
void ScalerELEDSetOvp(EnumLedBoosterStatus enumBoosterStatus)
{
    ...
    if(enumBoosterStatus == _ELED_BOOSTING_STATE)
    {
        ...
        for(ucOvpCount = 34; ucOvpCount <= 160; ucOvpCount += 2)
        {
        ...
        if(((WORD)_ELED_PCB_OVP_RATION * 160) < _ELED_PCB_OVP_TRIGGER_LEVEL)
        {
...
void ScalerELEDSetLedShort(void)
{
    BYTE ucAllShortCount = 0;
    BYTE ucAllShortParameter = 0;
    ...
    // Enable LED Short de-bounce
    ScalerSetBit(P16_CC_LEDBC_CODN5_4, ~_BIT7, _BIT7);      

    if(((WORD)((_PANEL_ELED_VOUT_TYPICAL * _PANEL_ELED_STRING_CURRENT * _PANEL_ELED_STRING_NUMBER) / (_ELED_PCB_VI_VOLTAGE)) > 1000) && (_PANEL_ELED_VOUT_TYPICAL > 35))
    {
        // Set LED_Short Tigger Voltage 3.5V & enable debounce
        ScalerSetBit(P16_CD_LEDBC_COND3_2, ~_BIT6, _BIT6);    
        ScalerSetBit(P16_D5_LEDBC_PROTECT_CTRL_1, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT1);     
    }
    else
    {
        if(_PANEL_ELED_STRING_CURRENT <= 100)   
        {
            // Set LED_Short Tigger Voltage 4.5V
            ScalerSetBit(P16_CD_LEDBC_COND3_2, ~_BIT6, _BIT6);    
            ScalerSetBit(P16_D5_LEDBC_PROTECT_CTRL_1, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1 | _BIT0));    
        }
        else if(_PANEL_ELED_STRING_CURRENT <= 120)
        {
            // Set LED_Short Tigger Voltage 4.3V
            ScalerSetBit(P16_CD_LEDBC_COND3_2, ~_BIT6, _BIT6);    
            ScalerSetBit(P16_D5_LEDBC_PROTECT_CTRL_1, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1));                            
        }
        else
        {
            // Set LED_Short Tigger Voltage 4.1V
            ScalerSetBit(P16_CD_LEDBC_COND3_2, ~_BIT6, _BIT6);    
            ScalerSetBit(P16_D5_LEDBC_PROTECT_CTRL_1, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT0));                
        }
    }
    ...
    // Check All LED Short Trigger Level
    for(ucAllShortCount = 30; ucAllShortCount <= 154; ucAllShortCount += 2)
    {
        if(((WORD)_ELED_PCB_OVP_RATION * ucAllShortCount) >= ((_PANEL_ELED_VOUT_TYPICAL - _PANEL_ELED_ALL_SHORT_VOLTAGE) * 100))
        {
            ScalerSetBit(P16_CD_LEDBC_COND3_2, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), ucAllShortParameter);
            break;
        }
...
void ScalerELEDSetLedShortEn(bit bOn)
{
    ...
    // Enable All LED Short FW Mode
    ScalerSetBit(P16_CD_LEDBC_COND3_2, ~_BIT7, 0x00);
    ...
    else
    {
        ...
        // Disable All LED Short Function
        ScalerSetBit(P16_CB_LEDBC_CODN7_6, ~_BIT4, 0x00);
    }
}
...
void ScalerELEDSetLedOpen(EnumLedBoosterStatus enumBoosterStatus)
{
    ...
    if(enumBoosterStatus == _ELED_BOOSTING_STATE)
    {
        ...
        if(((DWORD)_ELED_PCB_OVP_RATION * 160) < ((DWORD)_PANEL_ELED_VOUT_TYPICAL * 100))
        {
...
void ScalerELEDSetLedOpenEn(bit bOn)
{
    ...
    if(bOn == _ON)
    {
        ...
        // LED_OPEN De-bounce Time 17.8us
        ScalerSetBit(P16_C5_LEDBC_ADVREF3, ~(_BIT6 | _BIT5), (_BIT6 | _BIT5));
#endif
    }
    else
    {
...
void ScalerELEDBoosterVdetCompensation(void)
{
    ...
    // Vdet Voltage = 1.02V for samsung turn off big capacity
    pData[2] = 7;
