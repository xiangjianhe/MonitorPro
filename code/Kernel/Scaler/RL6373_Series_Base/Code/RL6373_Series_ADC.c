...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6373_Series_ADC.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6373_SERIES_ADC__
...
#warning "Not Apply the Solution in RL6373 Series!!"
#endif
...
void ScalerADCBiasCurrentSetting(void)
{
    ...
    // Select bandgap voltage 1.25V
    ScalerSetBit(P0_CC_ADC_V_BIAS0, ~(_BIT1 | _BIT0), _BIT0);    
}
...
{
    // Set ADC PGA
    ScalerSetBit(P0_CE_ADC_CTL_RGB, ~(_BIT7 | _BIT6 | _BIT5), ((ucAdcPGA & 0x07) << 5));
}
...
void ScalerADCSetClampToTopStatus(bit bEn)
{
    ...
    else
    {
        ...
        // Restore to RGB/YPbPr clamp, according to D-domain YUV2RGB block
        if(ScalerGetBit(P14_A1_SR_YCC2RGB_CTRL, _BIT0) == 0x00)
        {
            pData[0] = 0; // RGB            
        }
        else
...
void ScalerADCPGAbyPassSetting(void)
{
    // 6316 have no PGA by pass option, use for ADCNR setting
}
...
void ScalerADCNRSetting(void)
{
    // Alone Main Tone THD = 2
    ScalerSetBit(P2A_AC_ADCNR_THD1, ~(_BIT4 | _BIT3), _BIT3);

    // Setting Pattern Tolerance & Weighting
    ScalerSetByte(P2A_B0_ADCNR_PATTERN_TOLERANCE, 0x5F);
    ScalerSetBit(P2A_B1_ADCNR_DIFF_WEIGHTING, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT1 | _BIT0));

    // Tone Count Modify as High Gray THD set 32
    ScalerSetBit(P2A_AA_ADCNR_CONST1, ~(_BIT5 | _BIT4), 0x00);

    // Tone Count Modify Const. set 0
    ScalerSetBit(P2A_AB_ADCNR_CONST2, ~(_BIT3 | _BIT2), 0x00);

    // Total Frame Number set 2
    ScalerSetBit(P2A_AB_ADCNR_CONST2, ~(_BIT1 | _BIT0), (_BIT1 | _BIT0));

    // Enable New Function
    ScalerSetBit(P2A_AC_ADCNR_THD1, ~(_BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1));

    // Enable ADCNR
    ScalerSetByte(P2A_A0_ADCNR_CTRL1, 0x9C);
}
