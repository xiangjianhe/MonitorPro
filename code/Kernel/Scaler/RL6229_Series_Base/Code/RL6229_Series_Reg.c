...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6229_Series_Reg.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6229_SERIES_REG__
...
BYTE ScalerEnableAccessPort(WORD usAddr, BYTE ucValue)
{
    ...
    if(usAddr == CM_24_SCALE_DOWN_ACCESS_PORT)
    {
        *pucAddr |= _BIT7;
        ucValue = ucValue | _BIT7;
    }

    if((usAddr == CM_33_SU_ACCESS_PORT) || (usAddr == CM_64_CB_ACCESS_PORT))
    {
        *pucAddr |= _BIT7;
        ucValue = ucValue | (*pucAddr & 0xE0) | _BIT7;
    }
 
    if(usAddr == CM_60_HW_ACCESS_PORT)
    {
        *pucAddr |= _BIT7;        
...
void ScalerBurstWrite(BYTE *pAddress, WORD usWriteLength, BYTE ucBankNum, WORD usHostAddr, BYTE ucTableType, bit bTableLocation)
{
    ...
    MCU_BANK_SWITCH_CTRL_FFFC |= (_BIT5 & (((BYTE)bTableLocation) << 5));
    ...
    MCU_BANK_SWITCH_CTRL_FFFC &= ~_BIT5;
}
