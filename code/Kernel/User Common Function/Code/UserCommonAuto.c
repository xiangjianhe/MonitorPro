...
bit UserCommonAutoClockRoughAdjust(WORD *usCurrClock);
bit UserCommonAutoClockFineTuneAdjust(WORD *usCurrClock);
...
void UserCommonAutoConfig(void)
{
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_CHECK_MULTITIMES_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_CLOCK_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_PHASE_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_V_POSITION_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_H_POSITION_FINISH);
...
bit UserCommonAutoMeasureMultiTimes(void)
{
    ...
    {            
        ...
        {
            if(ScalerMeasureActiveRegion(_ANALOG_MODE_MEASURE, _NORMAL_MEASURE, _OVERSCAN_BOUNDARY, usCurrClock) == _FALSE)
            {
                return _FALSE;
            }
        }
...
#endif            
        {
            if(ScalerMeasureActiveRegion(_ANALOG_MODE_MEASURE, _NORMAL_MEASURE, _FULL_BOUNDARY, usCurrClock) == _FALSE)
            {
                return _FALSE;
            }
        }
...
bit UserCommonAutoClock(void)
{
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_CLOCK_CHECK_WIDTH_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_CLOCK_ROUGH_ADJUST_FINISH);
    ...
    UserInterfaceAutoConfigPercentage(_AUTO_CONFIG_CLOCK_FINETUNE_FINISH);
...
bit UserCommonAutoHPosition(void)
{
...
#if(_VGA_AUTO_CLOCK_ABORT_FIXED_POSITION == _ON)
    if((g_ucAutoConfigStatus & _AUTO_CONFIG_CLOCK_ABORT) == _AUTO_CONFIG_CLOCK_ABORT)
    {
        return _TRUE;
    }
#endif
    ...
    usDeltaIHWidth = ((UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IHWIDTH) - ScalerVgipGetCaptureHWidth()) / 2);

    if(GET_ACTIVE_H_WIDTH() <= UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IHWIDTH))
    {
        if((GET_ACTIVE_H_START() + usDeltaIHWidth) < (ScalerVgipGetCaptureHStartPosition() + ScalerVgipGetCaptureHDelay()))
        {
            g_stVGAModeUserData.usCenterHPosition = GET_ACTIVE_H_START();
        }
        else if(GET_ACTIVE_H_END() > (ScalerVgipGetCaptureHStartPosition() + ScalerVgipGetCaptureHDelay() + ScalerVgipGetCaptureHWidth() + usDeltaIHWidth - 1))
        {
            g_stVGAModeUserData.usCenterHPosition = GET_ACTIVE_H_END() - (ScalerVgipGetCaptureHWidth() + (usDeltaIHWidth << 1)) + 1;
        }
        else
        {
            return _TRUE;
...
bit UserCommonAutoVPosition(void)
{
    ...
    if(ScalerVgipGetCaptureVHeight() > UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IVHEIGHT))
    {
        usDeltaIVHeightUnderScan = ((ScalerVgipGetCaptureVHeight() - UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IVHEIGHT)) / 2);
    }
    else
    {
        usDeltaIVHeightOverScan = ((UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IVHEIGHT) - ScalerVgipGetCaptureVHeight()) / 2);
    }

    DebugMessageAnalog("7. wuzanne test : ModeTable VH", UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IVHEIGHT));
    DebugMessageAnalog("7. wuzanne test : Measure VH", GET_ACTIVE_V_HEIGHT());
    DebugMessageAnalog("7. wuzanne test : Measeure V Start", GET_ACTIVE_V_START());
    DebugMessageAnalog("7. wuzanne test : IVDelay", ScalerVgipGetCaptureVDelay());
    DebugMessageAnalog("7. wuzanne test : IVCapture", ScalerVgipGetCaptureVStartPosition());
    DebugMessageAnalog("7. wuzanne test : usDeltaIVHeightUnderScan", usDeltaIVHeightUnderScan);        
    DebugMessageAnalog("7. wuzanne test : CenterVposition", g_stVGAModeUserData.usCenterVPosition);
    DebugMessageAnalog("7. wuzanne test : Measure V End", GET_ACTIVE_V_END());
    
    if(GET_ACTIVE_V_HEIGHT() <= UserCommonModeSearchGetModetableTimingDataItem(g_stVGAModeUserData.ucModeNumber, _IVHEIGHT))
    {

        DebugMessageAnalog("7. wuzanne test : Picture under Size", 0);

        if((GET_ACTIVE_V_START() + usDeltaIVHeightOverScan) < (ScalerVgipGetCaptureVDelay() + ScalerVgipGetCaptureVStartPosition() + usDeltaIVHeightUnderScan))
        {
            DebugMessageAnalog("7. wuzanne test : Top Over", 0);

            g_stVGAModeUserData.usCenterVPosition = GET_ACTIVE_V_START();
        }
        else if((GET_ACTIVE_V_END() + usDeltaIVHeightUnderScan) > (ScalerVgipGetCaptureVDelay() + ScalerVgipGetCaptureVStartPosition() + (ScalerVgipGetCaptureVHeight() + usDeltaIVHeightOverScan) - 1))
        {
            DebugMessageAnalog("7. wuzanne test : Botten Over", 0);

            g_stVGAModeUserData.usCenterVPosition = GET_ACTIVE_V_END() - (ScalerVgipGetCaptureVHeight() + (2 * usDeltaIVHeightOverScan) - (2 * usDeltaIVHeightUnderScan))  + 1;
        }
        else
        {
            DebugMessageAnalog("7. wuzanne test : Do Nothing", 0);
    ...
    {
        DebugMessageAnalog("7. wuzanne test : Picture over Size", 0);
    ...
    {
        DebugMessageAnalog("7. wuzanne test : Center V Position Error", g_stVGAModeUserData.usCenterVPosition);
...
bit UserCommonAutoPhase(void) 
{
    ...
    {
        ScalerMeasureAutoFuncHBoundInitial((GET_ACTIVE_H_START() + 2), (GET_ACTIVE_H_END() - 2));       
        ScalerMeasureAutoFuncVBoundInitial((GET_ACTIVE_V_START() + 2), (GET_ACTIVE_V_END() - 2));
    }
    
    if(ScalerMeasureHWAutoPhaseSearch(_PHASE_SEARCH_STEP_4, &ucRoughPhase, &ulSODTemp1, &ulSODTemp2, _PHASE_SEARCH_WSOD, 0x80) == _FALSE)
    {
        return _FALSE;
    }
    ...
    if(ulSODTemp1 > 0)
    {
        ...
        {
            if(ScalerMeasureHWAutoPhaseSearch(_PHASE_SEARCH_STEP_4, &ucRoughPhase, &ulSODTemp1, &ulSODTemp2, _PHASE_SEARCH_WSOD, 0x40) == _FALSE)
            {
                return _FALSE;
            }
        }
    ...
    {           
        ...
        if(ScalerMeasureAutoPhaseGetSOD(&ulSODTemp1) == _FALSE)
        {
            return _FALSE;
        }    
...
bit UserCommonAutoClockRoughAdjust(WORD *usCurrClock)
{
    ...
    *usCurrClock = ((GET_INPUT_TIMING_HTOTAL()) >= g_stVGAModeUserData.usCenterClock) ? (GET_INPUT_TIMING_HTOTAL()) : g_stVGAModeUserData.usCenterClock;
    
    do
    {
        if(UserCommonAutoCheckClock(*usCurrClock) == _FALSE)
        {
            return _FALSE;
        }
        ...
        bAdjustPhaseResult = ScalerAPLLFastLockAdjust(*usCurrClock);
        ...
        PDATA_WORD(0) = ((DWORD)GET_INPUT_TIMING_HWIDTH() * (*usCurrClock) + GET_ACTIVE_H_WIDTH() / 2) / GET_ACTIVE_H_WIDTH();

        if(PDATA_WORD(0) == *usCurrClock)
        {            
            return _TRUE;
        }
        else
        {
            *usCurrClock = PDATA_WORD(0);
        }       
    }
    while(--ucCount);
...
bit UserCommonAutoClockFineTuneAdjust(WORD *usCurrClock)
{
    ...
    ScalerMeasureAutoFuncHBoundInitial(GET_ACTIVE_H_START(), GET_ACTIVE_H_END());
   
    ScalerMeasureAutoFuncVBoundInitial(GET_ACTIVE_V_START(), GET_ACTIVE_V_END());

    usRoughClock = *usCurrClock;
    
    usTargetClock = (*usCurrClock - ((_AUTO_TOLERANCE_PIXEL * GET_INPUT_TIMING_HTOTAL() / GET_INPUT_TIMING_HWIDTH()) + 1));

    usTargetClock = usTargetClock - ((usTargetClock % _AUTO_CLOCK_STEP));
  
    usBoundClock = (*usCurrClock + ((GET_INPUT_TIMING_HTOTAL() / GET_INPUT_TIMING_HWIDTH()) + 2));
    ...
    {                
        ...
        if(ScalerMeasureHWAutoPhaseSearch(_PHASE_SEARCH_STEP_8, &ucTempPhase, &ulSODMax, &ulSODMin, _PHASE_SEARCH_SOD, 0x40) == _FALSE)
        {
            return _FALSE;
        }

        DebugMessageAnalog("7. wuzanne test : ====== Target Clock ======", usTargetClock);
        DebugMessageAnalog("7. wuzanne test : SOG MAX", ulSODMax);
        DebugMessageAnalog("7. wuzanne test : SOG MIN", ulSODMin);
        DebugMessageAnalog("7. wuzanne test : SOG Ratio", (ulSODMax / ulSODMin));
        DebugMessageAnalog("7. wuzanne test : Valid SOD Threshold", (((DWORD)GET_INPUT_TIMING_HWIDTH() * (DWORD)GET_INPUT_TIMING_VHEIGHT() * (DWORD)0x40) >> 7));
        
        if(ulSODMax > ulSODMaxValue)
        {
            *usCurrClock = usTargetClock;
            ulSODMaxValue = ulSODMax;
        ...
        UserInterfaceAutoConfigPercentage((_AUTO_CONFIG_CLOCK_ROUGH_ADJUST_FINISH + (ucPercentageStep * ucCounter)));
    }
    ...
    if(ulSODMaxValue <= (((DWORD)GET_INPUT_TIMING_HWIDTH() * (DWORD)GET_INPUT_TIMING_VHEIGHT() * (DWORD)0x40) >> 8))
    {
        ...
        {
            *usCurrClock = usRoughClock;
        }
        else
        {        
            if(((usRoughClock + (usRoughClock % _AUTO_CLOCK_STEP)) % 4) == 0x00)
            {
                *usCurrClock = (usRoughClock + (usRoughClock % _AUTO_CLOCK_STEP));
            }
            else
            {
                *usCurrClock = (usRoughClock - (usRoughClock % _AUTO_CLOCK_STEP));
    ...
    {               
        ...
        if(ScalerMeasureHWAutoPhaseSearch(_PHASE_SEARCH_STEP_4, &ucTempPhase, &ulSODMax, &ulSODMin, _PHASE_SEARCH_SOD, 0xA0) == _FALSE)
        {
            return _FALSE;
        }       
        ...
        else 
        {
            ...
            DebugMessageAnalog("7. wuzanne test : Double Check : Clock", *usCurrClock);
            DebugMessageAnalog("7. wuzanne test : Double Check SODMax", ulSODMax);
            DebugMessageAnalog("7. wuzanne test : Double Check SODMin", ulSODMin);
            DebugMessageAnalog("7. wuzanne test : Double Check : ulSODMaxMinRatio", ulSODMaxMinRatio);
            DebugMessageAnalog("7. wuzanne test : Double Check Threshold", (((DWORD)GET_INPUT_TIMING_HWIDTH() * (DWORD)GET_INPUT_TIMING_VHEIGHT() * (DWORD)0xA0) >> 9));
...
void UserCommonAutoPhaseTestFunction(void)
{
    ...
    for(ucCounter=0; ucCounter < 64; ucCounter++)
    {
        ...
        ScalerMeasureAutoPhaseGetSOD(&ulSODValue);

        DebugMessageAnalog("7. wuzanne test : SOD", ulSODValue);
    }
}
*/
