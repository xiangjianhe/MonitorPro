...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6336_Series_Mcu.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6336_SERIES_MCU__
...
//--------------------------------------------------
#define _DDCRAM_DDC1_MAX_ADDRESS                            0xFD00
#define _DDCRAM_DDC2_MAX_ADDRESS                            0xFD80

#define _EMBEDDED_DDCRAM_DDC2_ADDRESS                       (((0xFE00 - _EMBEDDED_DDCRAM_D1_SIZE) > _DDCRAM_DDC2_MAX_ADDRESS) ? (_DDCRAM_DDC2_MAX_ADDRESS) : (0xFE00 - _EMBEDDED_DDCRAM_D1_SIZE))
...
volatile BYTE xdata MCU_ADC_SEG_ADDR_FF19        _at_ 0xFF19;
volatile BYTE xdata MCU_ADC_SEG_DATA_FF1A        _at_ 0xFF1A;
...
volatile BYTE xdata MCU_EWSR_OP_CODE_FF63        _at_ 0xFF63;  
...
void ScalerMcuPwmControl(EnumPowerStatus enumPowStatus)
{
    ...
    {   
        ...
        MCU_I2C_PWM_CLK_STOP_FFF2 |= (_BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_I2C_PWM_CLK_STOP_FFF2 &= ~(((BYTE)_PWM3_EN << 3) | ((BYTE)_PWM2_EN << 2) | ((BYTE)_PWM1_EN << 1) | ((BYTE)_PWM0_EN));
        
        // PWM Output Enable
        MCU_PWM_EN_FF48 &= ~(_BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_PWM_EN_FF48 |= (((BYTE)_PWM3_EN << 3) | ((BYTE)_PWM2_EN << 2) | ((BYTE)_PWM1_EN << 1) | ((BYTE)_PWM0_EN));
    ...
    else if(enumPowStatus == _POWER_STATUS_SAVING)
    {
        // PWM Output Enable
        MCU_PWM_EN_FF48 &= ~(_BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_PWM_EN_FF48 |= (((BYTE)_PWM3_PSAV_EN << 3) | ((BYTE)_PWM2_PSAV_EN << 2) | ((BYTE)_PWM1_PSAV_EN << 1) | ((BYTE)_PWM0_PSAV_EN));

        // PWM output clock stop disable
        MCU_I2C_PWM_CLK_STOP_FFF2 |= (_BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_I2C_PWM_CLK_STOP_FFF2 &= ~(((BYTE)_PWM3_PSAV_EN << 3) | ((BYTE)_PWM2_PSAV_EN << 2) | ((BYTE)_PWM1_PSAV_EN << 1) | ((BYTE)_PWM0_PSAV_EN));  
    }
    else        
    {    
