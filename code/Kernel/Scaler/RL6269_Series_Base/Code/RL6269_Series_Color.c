...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6269_Series_Color.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6269_SERIES_COLOR__
...
#define _RINGING_FILTER_THD_TANGE           4
...
void ScalerColorRingingFilterAdjust(BYTE *pucOffsetCoef)
{
    ...
    else if(ucTHD > 255 - (1 << _RINGING_FILTER_THD_TANGE))
    {
        ucTHD = 255 - (1 << _RINGING_FILTER_THD_TANGE);
    }

    ucRFEn = 1;
    
    ScalerSetByte(P25_A1_RFILTER_THD, ucTHD);
    ScalerSetByte(P25_A2_RFILTER_THD_RNG_EXP, _RINGING_FILTER_THD_TANGE);
...
bit ScalerColorGetIColorConvertStatus(void)
{
    // Check if Color Conversion / IDLTI are all disabled
    if((ScalerGetBit(CM_9C_YUV2RGB_CTRL, _BIT0) == _OFF) && (ScalerGetBit(P11_A1_I_DLTI_CTRL_0, _BIT0) == _OFF))
    {
        return _OFF;
    }
    else
...
void ScalerColorDCROnOff(bit bOnOff)
{
    if(bOnOff == _OFF)
    {        
        ScalerSetBit(P7_DA_DCR_CONTROL, ~(_BIT1 | _BIT0), 0x00);
    }
    else
    {
        if(ScalerGetBit(P7_DA_DCR_CONTROL, _BIT0) == 0x00)
        {
            // Set DCR enable
            ScalerSetBit(P7_DA_DCR_CONTROL, ~(_BIT0), _BIT0);
...
void ScalerColorDCRThresholdAdjust(BYTE ucThreshold1, BYTE ucThreshold2)
{
    ScalerSetByte(P7_DE_DCR_THESHOLD1, ucThreshold1);
    ScalerSetByte(P7_DF_DCR_THESHOLD2, ucThreshold2);
}
...
DWORD ScalerColorDCRReadResult(EnumDCRAdjOption enumAdjustOption)
{
    ScalerSetBit(P7_DA_DCR_CONTROL, ~(_BIT1), _BIT1);
    
    switch(enumAdjustOption)
    {
        case _DCR_ABOVE_TH1_NUM:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_00_DCR_ABOVE_TH1_NUM2);
            ScalerRead(P7_D9_DCR_DATA_PORT, 3, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 8;
            break;

        case _DCR_ABOVE_TH1_VAL:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_03_DCR_ABOVE_TH1_VAL3);
            ScalerRead(P7_D9_DCR_DATA_PORT, 4, pData, _NON_AUTOINC);
            break;

        case _DCR_ABOVE_TH2_NUM:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_07_DCR_ABOVE_TH2_NUM2);
            ScalerRead(P7_D9_DCR_DATA_PORT, 3, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 8;
            break;

        case _DCR_ABOVE_TH2_VAL:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_0A_DCR_ABOVE_TH2_VAL3);
            ScalerRead(P7_D9_DCR_DATA_PORT, 4, pData, _NON_AUTOINC);
            break;

        case _HIGH_LV_NUM_R:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_0E_DCR_HIGH_LV_NUM_R1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _LOW_LV_NUM_R:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_10_DCR_LOW_LV_NUM_R1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _HIGH_LV_VAL_R:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_12_DCR_HIGH_LV_VAL_R);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;

        case _LOW_LV_VAL_R:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_13_DCR_LOW_LV_VAL_R);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;

        case _HIGH_LV_NUM_G:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_14_DCR_HIGH_LV_NUM_G1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _LOW_LV_NUM_G:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_16_DCR_LOW_LV_NUM_G1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _HIGH_LV_VAL_G:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_18_DCR_HIGH_LV_VAL_G);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;

        case _LOW_LV_VAL_G:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_19_DCR_LOW_LV_VAL_G);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;

        case _HIGH_LV_NUM_B:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_1A_DCR_HIGH_LV_NUM_B1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _LOW_LV_NUM_B:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_1C_DCR_LOW_LV_NUM_B1);
            ScalerRead(P7_D9_DCR_DATA_PORT, 2, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 16;
            break;

        case _HIGH_LV_VAL_B:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_1E_DCR_HIGH_LV_VAL_B);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;

        case _LOW_LV_VAL_B:
            ScalerSetByte(P7_D8_DCR_ACCESS_PORT, _P7_D9_PT_1F_DCR_LOW_LV_VAL_B);
            ScalerRead(P7_D9_DCR_DATA_PORT, 1, pData, _NON_AUTOINC);
            PDATA_DWORD(0) = PDATA_DWORD(0) >> 24;
            break;
    }

    ScalerSetBit(P7_DA_DCR_CONTROL, ~_BIT1, 0x00);
...
void ScalerColorICMLoadTable(BYTE *pICMTable, BYTE ucBankNum, BYTE ucCMSel)
{
    ...
    ScalerBurstWrite(&pICMTable[_ICM_TABLE_SIZE], _ICM_BRI_TABLE_SIZE, ucBankNum, P7_D3_ICM_DATA_PORT, _BURSTWRITE_DATA_COMMON, _BURSTWRITE_FROM_FLASH);
#else
    ScalerSetByte(P7_D2_ICM_ACCESS_PORT, 0x67);
    ScalerSetByte(P7_D3_ICM_DATA_PORT, 0x00);    
#endif  
}
...
void ScalerColorSixColorEnable(void)
{
    ...
    ScalerSetByte(P7_CC_SCM_BLOCK_ENABLE, 0x00);  // Close SCM 
...
void ScalerColorUltraVividOff(void)
{
    ...
    //Disable SHP
    ScalerSetBit(P12_A0_SR_SHP_CTRL, ~_BIT7, 0x00);

    //For double buffer
    ScalerVgipDoubleBufferApply();

    // Wait IVS & DVS for Ultravivid Settings to become effective
...
{    
    if((ScalerGetBit(P14_C0_LSR_CTRL, _BIT7) == 0x00) &&
       (ScalerGetBit(P11_A1_I_DLTI_CTRL_0, _BIT0) == 0x00) &&
       (ScalerGetBit(P12_A0_SR_SHP_CTRL, _BIT7) == 0x00))
    {
        return _OFF;
    }
    else
...
{     
    ...
    if(GET_V_SCALE_UP() == _TRUE)
    {
        ...
        {
            ScalerBurstWrite(pUltraVividSetting1, 172, ucBankNum, P11_B9_SCALER_PEAKING_C0, _BURSTWRITE_COMMAND, _BURSTWRITE_FROM_FLASH);
        }
        else    // IVHeight/DVHeight <= 1
        {
            ScalerBurstWrite(pUltraVividSetting2, 172, ucBankNum, P11_B9_SCALER_PEAKING_C0, _BURSTWRITE_COMMAND, _BURSTWRITE_FROM_FLASH);
        }                
    }
    else if(GET_V_SCALE_DOWN() == _TRUE)
    {
        ScalerBurstWrite(pUltraVividSetting3, 172, ucBankNum, P11_B9_SCALER_PEAKING_C0, _BURSTWRITE_COMMAND, _BURSTWRITE_FROM_FLASH);
    }
    else // by pass
    {
        ScalerBurstWrite(pUltraVividSetting4, 172, ucBankNum, P11_B9_SCALER_PEAKING_C0, _BURSTWRITE_COMMAND, _BURSTWRITE_FROM_FLASH);
    }      
...
void ScalerColorHLWAdjust(WORD usHPos, WORD usHWidth, WORD usVPos, WORD usVHeight)
{
    ...
    ScalerGetDataPortByte(CM_60_HW_ACCESS_PORT, _CM_61_PT_08_HW_BORDER_WIDTH, 1, &pData[14], _AUTOINC);    
    pData[15] = (pData[14] & 0x0F);  // Border Width
    ...
    if(((pData[14] & _BIT4) == _BIT4) && (usVPos + usVHeight + pData[15] > _PANEL_DV_HEIGHT))
    {
        pData[6] = (BYTE)(((_PANEL_DV_HEIGHT - pData[15]) >> 8) & 0x0F);
        pData[7] = (BYTE)((_PANEL_DV_HEIGHT - pData[15]) & 0xFF);        
    }
    else
    {        
...
void ScalerColorHLWBorderAdjust(BYTE ucBorderEn, BYTE ucBorderWidth, BYTE ucRed, BYTE ucGreen, BYTE ucBlue)
{
    ...
    ScalerGetDataPortByte(CM_60_HW_ACCESS_PORT, _CM_61_PT_06_HW_V_END_H, 2, pucTemp, _AUTOINC);    

    if(((ucBorderEn & _BIT4) == _BIT4) && (((WORD *)pucTemp)[0] + ucBorderWidth > _PANEL_DV_HEIGHT))
    {
        ScalerSetDataPortByte(CM_60_HW_ACCESS_PORT, _CM_61_PT_06_HW_V_END_H, ((_PANEL_DV_HEIGHT - ucBorderWidth) >> 8) & 0x0F);
        ScalerSetDataPortByte(CM_60_HW_ACCESS_PORT, _CM_61_PT_07_HW_V_END_L, (_PANEL_DV_HEIGHT - ucBorderWidth) & 0xFF);    
    }
}
