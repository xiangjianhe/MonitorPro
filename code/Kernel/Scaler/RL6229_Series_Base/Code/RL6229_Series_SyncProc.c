...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6229_Series_SyncProc.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6229_SERIES_SYNCPROC__
...
void ScalerSyncProcPulseDetectEnable(BYTE ucEnable)
{
    if(ucEnable == _ENABLE)
    {
        // VS detect is not used
        ScalerSetDataPortBit(CM_5C_SYNC_PROC_ACCESS_PORT, _CM_5D_PT_19_HS_DETECT_SETTING, ~(_BIT7 | _BIT6 | _BIT4), (_BIT7 | _BIT6));
    }
    else
    {
        ScalerSetDataPortBit(CM_5C_SYNC_PROC_ACCESS_PORT, _CM_5D_PT_19_HS_DETECT_SETTING, ~(_BIT7 | _BIT6 | _BIT4), 0x00);
    }
}
