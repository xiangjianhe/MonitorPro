...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6230_Series_Mcu.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6230_SERIES_MCU__
...
//--------------------------------------------------
#define _EMBEDDED_DDCRAM_DDC1_ADDRESS                       0xFC00

//--------------------------------------------------
// Definitions DDCCI Protocol Items
//--------------------------------------------------
#define _DDCCI_BUFFER_SIZE                                  16
...
volatile BYTE xdata MCU_ADC_SEGMENT_ADDRESS      _at_ 0xFF19;
...
volatile BYTE xdata MCU_PORT50_PIN_REG_FFC5      _at_ 0xFFC5;
volatile BYTE xdata MCU_PORT51_PIN_REG_FFC6      _at_ 0xFFC6;
...
volatile BYTE xdata MCU_PORT82_PIN_REG_FFD8      _at_ 0xFFD8;
volatile BYTE xdata MCU_PORT83_PIN_REG_FFD9      _at_ 0xFFD9;
volatile BYTE xdata MCU_PORT84_PIN_REG_FFDA      _at_ 0xFFDA;
volatile BYTE xdata MCU_PORT85_PIN_REG_FFDB      _at_ 0xFFDB;
volatile BYTE xdata MCU_PORT86_PIN_REG_FFDC      _at_ 0xFFDC;
volatile BYTE xdata MCU_PORT87_PIN_REG_FFDD      _at_ 0xFFDD;
...
volatile BYTE xdata MCU_PWM_I2C_CLOCK_STOP_FFF1  _at_ 0xFFF1;
...
void ScalerMcuGpioInitial(void)
{
    ...
    // Source selection for P1, P3, P5, P6, P8 read back from bus value     
    MCU_PORT_READ_CTRL_FFC0 |= 0x2F;
}
...
void ScalerMcuIspInitial(void)
{
	...
	// Disable SOD XDATA access to XRAM
    MCU_XDATA_SPI_WR_PROT_FFF0 &= ~_BIT7; 
}
...
{
	// DDCRAM partition
#if(_EMBEDDED_DDCRAM_A0_SIZE == 0x80)
    MCU_DDCRAM_PART_FF21 &= ~_BIT1;
#elif(_EMBEDDED_DDCRAM_A0_SIZE == 0x100)
    MCU_DDCRAM_PART_FF21 |= _BIT1;
#endif

    // Modify DDC Segment Slave Address for DDC1
    MCU_ADC_SEGMENT_ADDRESS = 0xFF;
...
void ScalerMcuPwmControl(EnumPowerStatus enumPowStatus)
{
    ...
    {   
        ...
        MCU_PWM_I2C_CLOCK_STOP_FFF1 |= (_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_PWM_I2C_CLOCK_STOP_FFF1 &= ~(((BYTE)_PWM5_EN << 5) | ((BYTE)_PWM4_EN << 4) | ((BYTE)_PWM3_EN << 3) | ((BYTE)_PWM2_EN << 2) | ((BYTE)_PWM1_EN << 1) | ((BYTE)_PWM0_EN)); 
    ...
    else if(enumPowStatus == _POWER_STATUS_SAVING)
    {
        ...
        // PWM output clock stop disable
        MCU_PWM_I2C_CLOCK_STOP_FFF1 |= (_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0);
        MCU_PWM_I2C_CLOCK_STOP_FFF1 &= ~(((BYTE)_PWM5_PSAV_EN << 5) | ((BYTE)_PWM4_PSAV_EN << 4) | ((BYTE)_PWM3_PSAV_EN << 3) | ((BYTE)_PWM2_PSAV_EN << 2) | ((BYTE)_PWM1_PSAV_EN << 1) | ((BYTE)_PWM0_PSAV_EN));  
    }
    else        
    {    
