...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6230_Series_LDO.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6230_SERIES_LDO__
...
{  
    ...
    {
        // Disable BJT Control
        ScalerSetBit(P0_DF_EBD_REGLATOR_A_CTRL, ~_BIT7, 0x00);

        // Set LDO voltage output = 1.15V ==> mapping actual 1.2V
        ScalerSetBit(P0_E0_EBD_REGLATOR_DB_CTRL, ~(_BIT2 | _BIT1 | _BIT0), (_BIT1 | _BIT0));

        switch(enumPorStatus)
        {
            case _POWER_STATUS_NORMAL:
                // Set opamp current to normal
                ScalerSetBit(P0_E0_EBD_REGLATOR_DB_CTRL, ~(_BIT5 | _BIT4 | _BIT3), 0x00);
                
                // Set to LDO max driving current, can driving 200mA
                ScalerSetBit(P0_E0_EBD_REGLATOR_DB_CTRL, ~_BIT6, _BIT6);
            ...
            case _POWER_STATUS_SAVING:
                ...
                ScalerSetBit(P0_E0_EBD_REGLATOR_DB_CTRL, ~(_BIT5 | _BIT4 | _BIT3), (_BIT5 | _BIT3));
                               
                // Set to LDO low driving current, can driving 60mA
                ScalerSetBit(P0_E0_EBD_REGLATOR_DB_CTRL, ~_BIT6, 0x00);
