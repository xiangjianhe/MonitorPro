...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6229_Series_DP.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6229_SERIES_DP__
...
void ScalerDpPhyInitial(void)
{
    ...
    // [1] TP1 Signal to Lock FLD After LEQ
    // [0] Enable Servo Loop Adaptation
    ScalerSetBit(PB_A4_CDR_2, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT6 | _BIT4 | _BIT0));
    ...
    // Enable High DC Gain 
    ScalerSetBit(PB_B1_EQ_1, ~_BIT5, _BIT5);
...
void ScalerDpSetDFEInitial_EXINT0(void) using 1
{
    ...
    ScalerSetBit_EXINT0(P1D_D0_L3_DFE_EN_1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6));
...
void ScalerDpSetDFEInitial(EnumDpLinkRate enumDpLinkRate)
{
    ...
    // [3] Disable Running Length Detection
    // [2] bypass Blank out Case-3 Symbol
    ScalerSetBit(P1D_A0_L0_DFE_EN_1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6));
    ScalerSetBit(P1D_B0_L1_DFE_EN_1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6));
    ScalerSetBit(P1D_C0_L2_DFE_EN_1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6));
    ScalerSetBit(P1D_D0_L3_DFE_EN_1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT7 | _BIT6));
...
bit ScalerDpStreamClkRegenerate(void)
{
    ...
    if((400000000 > ulStreamClkValue) && (ulStreamClkValue >= 270000000))
    {
        ...
        ulpllm = (((((ulStreamClkValue * ucdiv_divs * ucdiv_pllo) / 1000) * ucplln) + (ulXtal_clk / 4)) / (ulXtal_clk));
    ...
    // Set Phase Swallow Path to Path 1  _BIT2 : 0 --> Path 2  1--> Path 1
    ScalerSetDataPortBit(PC_B3_DP_ACCESS_PORT, _PC_B4_PT_72_MN_SCLKG_PLL_WD, ~(_BIT2), _BIT2);

    // Force to Increase Vco Freq in order to phase Swallow up (Frequency output going down)
    ulpllm++;

    if(ScalerGetDataPortBit(PC_B3_DP_ACCESS_PORT, _PC_B4_PT_72_MN_SCLKG_PLL_WD, _BIT2) == _BIT2)
    {
        // Path1 Fvco Formula: Fvco = xtal * M / N * (1 - offset / 2^21))
        // offset = (m/n*f_ref - f)/(m/n*f_ref)*2^21
        // seperate *2^21 to *32*16384*4
        ulOffsetValue = (((ulXtal_clk * 1000 / ucplln) * ulpllm) - (ulStreamClkValue * ucdiv_pllo * ucdiv_divs)) * 32 / ulpllm * ucplln / ulXtal_clk * 16384 / 1000 * 4;
    }
    else
    {
        // Path2 Fvco Formula: Fvco = xtal * M / (N * (1 + offset / 2^21))
        // ==> offset = (xtal * M / N - Streamclk * O * S) / Sreamclk / O / S
        ulOffsetValue = (((ulXtal_clk * 1000 / ucplln) * ulpllm) - (ulStreamClkValue * ucdiv_pllo * ucdiv_divs)) * 32 / (ulStreamClkValue / 1000) / ucdiv_pllo / ucdiv_divs * 16384 / 1000 * 4;
    }

    // Calculate 2's complement
    ulOffsetValue = 0x7FFFF - ulOffsetValue + 1;
    ...
    ScalerSetDataPortBit(PC_B3_DP_ACCESS_PORT, _PC_B4_PT_72_MN_SCLKG_PLL_WD, ~(_BIT6 |_BIT5 | _BIT4 | _BIT3 | _BIT1 | _BIT0) , (_BIT4 | _BIT1));
    ...
    ScalerSetDataPortBit(PC_B3_DP_ACCESS_PORT, _PC_B4_PT_53_MN_SCLKG_OFFS_H, ~(_BIT2 | _BIT1 | _BIT0),    (((BYTE)(ulOffsetValue >> 16)) & (_BIT2 | _BIT1 | _BIT0)));
...
{    
    ...
    ScalerSetBit(P2_E0_AUX_PS_CTRL, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT0));  
