...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6001_Series_ADC.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6001_SERIES_ADC__
...
#warning "Not Apply the Solution in RL6001 Series!!"
#endif
...
void ScalerADCBandgap(bit bSelect)
{
    ...
    {               
        // ADC Bandgap On
        ScalerSetBit(P0_C6_ADC_POWER, ~_BIT3, _BIT3);

        // Bit3: ADC Regulator Output Not Short to Ground
    ...
    else
    {
        ...
        ScalerSetBit(P0_D5_ADC_CLAMP_CTRL1, ~(_BIT3 | _BIT2 | _BIT0), _BIT0); 

        // ADC Bandgap Off
        ScalerSetBit(P0_C6_ADC_POWER, ~_BIT3, 0x00);
    }
}
...
void ScalerADCOutputClkSetting(void)
{
    // Clock polarity select positive
    ScalerSetBit(P0_BB_ADC_2X_SAMPLE, ~_BIT5, _BIT5);

    // Output clock divider invert, Duty Staiblizer is 50%
    ScalerSetByte(P0_BC_ADC_CLOCK, 0x41);
}
...
void ScalerADCPowerSetting(void)
{
    // VCM Power ADC Clock Power On, SOG_ADC0 Power On, R/G/B ADC Power On
    ScalerSetBit(P0_C6_ADC_POWER, ~(_BIT7 | _BIT6 | _BIT5 | _BIT2 | _BIT1 | _BIT0), (_BIT7 | _BIT6 | _BIT5 | _BIT2 | _BIT1 | _BIT0));
}
...
void ScalerADCBiasCurrentSetting(void)
{
    ...
    ScalerSetBit(P0_C7_ADC_I_BAIS0, ~(_BIT7 | _BIT6), (_BIT7 | _BIT6));
    ScalerSetBit(P0_C8_ADC_I_BAIS1, ~(_BIT7 | _BIT6), _BIT7);

    // SOG DAC current select 25u
    ScalerSetBit(P0_C9_ADC_I_BAIS2, ~(_BIT4 | _BIT3 | _BIT2), _BIT2);    

    // PGA Input offset Current 480uA
    ScalerSetBit(P0_CC_ADC_V_BIAS0, ~(_BIT7 | _BIT6 | _BIT3 | _BIT2), (_BIT7 | _BIT6));    

    // PGA Input GM Power On
    ScalerSetByte(P0_CD_ADC_V_BIAS1, 0x09);
}
...
void ScalerADCSetClampToTopStatus(bit bEn)
{
    ...
    else
    {
        ...
        // Restore to RGB/YPbPr clamp, according to I-domain YUV2RGB block
        if(ScalerGetBit(CM_9C_YUV2RGB_CTRL, _BIT0) == 0x00)
        {
            pData[0] = 0; // RGB            
        }
        else
...
void ScalerADCBandWidthSetting(DWORD ulAdcClock)
{    
    // Set PGA LPF parameter according differential pixel clock to reduce high frequency noise
    if(ulAdcClock < 38)
    {
        // 75MHz
        ScalerSetBit(P0_CE_ADC_CTL_RGB, ~(_BIT1 | _BIT0), 0x00); 
    }
    else if(ulAdcClock < 68)
    {
        // 150MHz
        ScalerSetBit(P0_CE_ADC_CTL_RGB, ~(_BIT1 | _BIT0), _BIT0);
    }
    else
    {
        // 300MHz
        ScalerSetBit(P0_CE_ADC_CTL_RGB, ~(_BIT1 | _BIT0), _BIT1);          
    }

    // Set Bias current according differential pixel clock.
    if(ulAdcClock < 50)
    {
        ScalerSetByte(P0_CA_ADC_I_BAIS3, 0x5A);
    }
    else if(ulAdcClock < 200)
    {
...
void ScalerADCDCRestoreEn(bit bEn)
{
    if(bEn)
    {
        // Enable SOG DC restore
        ScalerSetBit(P0_D3_ADC_DCR_CTRL, ~_BIT4, _BIT4);
    }
    else
    {
        // Disable SOG DC restore
...
void ScalerADCPGAbyPassSetting(void)
{
    // 6001 have no PGA by pass option
}
