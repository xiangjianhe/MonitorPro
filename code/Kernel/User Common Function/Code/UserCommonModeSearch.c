...
bit UserCommonModeSearchCompareIVHeight(BYTE ucLastModeIndex, BYTE ucCurrModeIndex, BYTE *pModeBestInx);
#endif // #if(_MODE_SEARCH_BY_ACTIVE_MEASURE_SUPPORT == _ON)

WORD UserCommonModeSearchGetHeightByGTF(WORD usVTotal, WORD usVFreq);
bit UserCommonModeSearchCheckHFreq(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchCheckVFreq(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchCheckGTFHeight(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchHVSyncPulseCheck(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchCompareByPolarity(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchCompareByInterlace(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
bit UserCommonModeSearchCompareByVideoModeFlg(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx);
...
bit UserCommonModeSearchCheckFIFOSetting(VGAModeUserDataType stFifoUserData, BYTE ucModeIndex);
WORD UserCommonModeSearchGetModetableTimingDataItem(BYTE ucModeNumber, EnumTimingInfoSelect enumTimingInfo);
bit UserCommonModeSearchCheckPresetModeTable(BYTE ucModeNumber);
...
BYTE UserCommonModeSearchModeVGA(void)
{
    ...
    if(ucModeIndex < _MAX_PRESET_MODE)
    {
        ...
        ucModeIndex = UserInterfaceCheckVGAModeSearch(ucModeIndex);
...
bit UserCommonModeSearchOORModeByMeasureInfo(void)
{
    ...
    || (GET_INPUT_TIMING_VFREQ() < _OOR_V_FREQ_LOWER_BOUND) || (GET_INPUT_TIMING_VFREQ() > _OOR_V_FREQ_UPPER_BOUND))
    {
        return _TRUE;
    }
...
{  
    ...
    if(((DWORD)abs(GET_INPUT_TIMING_VFREQ() - tINPUTMODE_PRESET_TABLE[ucModeIndex].usIVFreq) * 1000) > ((1000 + (((WORD)abs(ScalerLCTankGetClock() - _XTAL14318K) * 1000) / _XTAL14318K)) * tINPUTMODE_PRESET_TABLE[ucModeIndex].ucIVFreqTolerance))
    {
        return _FALSE;
    }

    if(((DWORD)abs(GET_INPUT_TIMING_HFREQ() - tINPUTMODE_PRESET_TABLE[ucModeIndex].usIHFreq) * 1000) > ((1000 + (((WORD)abs(ScalerLCTankGetClock() - _XTAL14318K) * 1000) / _XTAL14318K)) * tINPUTMODE_PRESET_TABLE[ucModeIndex].ucIHFreqTolerance))
    {
        return _FALSE;
    }
#else
...
bit UserCommonModeSearchHVSyncPulseCheck(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx)
{
    ...
    while(tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode != _MODE_NOSUPPORT)
    {
            ...
            && ((ucCurrentModeCnt == tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode) || (ucCurrentModeCnt == tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucSecondMode)))
        {
            ...
            {
                *pModeBestInx = tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode;

                DebugMessageAnalog("2. Detect By H-Sync Pulse", *pModeBestInx);
                
                return _TRUE;
            }
            else
            {
                *pModeBestInx = tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucSecondMode;
    ...
    while(tVSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode != _MODE_NOSUPPORT)
    {
            ...
            && ((ucCurrentModeCnt == tVSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode) || (ucCurrentModeCnt == tVSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucSecondMode)))
        {
            ...
            {
                *pModeBestInx = tVSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode;

                DebugMessageAnalog("2. Detect By V-Sync Pulse", *pModeBestInx);

                return _TRUE;
            }
            else
            {
                *pModeBestInx = tVSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucSecondMode;
...
bit UserCommonModeSearchCompareByPolarity(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx)
{
    ...
    while(tPOLARITY_MODEGROUP_TABLE[ucCnt].ucFirstMode != _MODE_NOSUPPORT)
    {
        ...
        {            
            ...
            {
                *pModeBestInx = tPOLARITY_MODEGROUP_TABLE[ucCnt].ucFirstMode;   

                return _TRUE;
            }
            else if((bit)((tINPUTMODE_PRESET_TABLE[tPOLARITY_MODEGROUP_TABLE[ucCnt].ucSecondMode].ucMiscFlag & 0x0F) & ucPolarity) == _TRUE)
            {
                *pModeBestInx = tPOLARITY_MODEGROUP_TABLE[ucCnt].ucSecondMode;
...
bit UserCommonModeSearchCompareByInterlace(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx)
{
    ...
    while(tINTERLACE_MODEGROUP_TABLE[ucCnt].ucFirstMode != _MODE_NOSUPPORT)
    {
        ...
        {        
            ...
            {
                *pModeBestInx = tINTERLACE_MODEGROUP_TABLE[ucCnt].ucFirstMode;   
            ...
            {
                *pModeBestInx = tINTERLACE_MODEGROUP_TABLE[ucCnt].ucSecondMode;
...
bit UserCommonModeSearchCompareByVideoModeFlg(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pModeBestInx)
{
    ...
    {
        if(ScalerSyncVgaGetSyncPad() == _TRUE)
        {
            *pModeBestInx =  ucLastModeCnt;
        }
        else
...
void UserCommonModeSearchUpdateInputDataRefFIFO(BYTE ucFifoIndex, BYTE ucIndex)
{
    ...
    if((UserCommonModeSearchCheckFIFOSetting(g_stVGAModeUserData, ucIndex) == _FALSE) || (UserInterfaceLoadModeTableDefault() == _TRUE))
    {
...
bit UserCommonModeSearchCheckFIFOSetting(VGAModeUserDataType stFifoUserData, BYTE ucModeIndex)
{
    WORD usIHCaptureStart = 0;
    WORD usClockDelta1 = 0;
    WORD usClockDelta2 = 0;
    WORD usClockDelta = 0;
    WORD usClockMax = 0;
    WORD usClockMin = 0;

    usIHCaptureStart = ScalerIDomainVgipIHCaptureMaxEstimate(stFifoUserData.usCenterHPosition);
