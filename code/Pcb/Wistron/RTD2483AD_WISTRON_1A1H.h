...
//----------------------------------------------------------------------------------------------------
// ID Code      : RTD2483AD_WISTRON_PCB.h No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
#if(_PCB_TYPE == _RTD2483AD_WISTRON_1A1H)
...
#define _D1_INPUT_PORT_GROUP                    _INPUT_PORT_GROUP_2
...
#define _AUDIO_VOLUME_PWM                       _PWM0            
...
//--------------------------------------------------
// Definitions for DDC Switch GPIO
//--------------------------------------------------
#define _DDC_SWITCH_GPIO_SUPPORT                _ON
...
// POWER_KEY_EN
#define _PIN_39                                 (1 & 0x03) // Page 10-0xB8[1:0]
// 0 ~ 3 (0: PDD0i<I>, 1: PDD0o<PP>, 2: PDD0o<OD>, 3: TCON14<O>)

// DDC WP Pin
#define _PIN_40                                 (1 & 0x03) // Page 10-0xB9[1:0]
...
// ATTN
#define _PIN_52                                 (0 & 0x03) // Page 10-0xA4[7:6]
...
// SYSTEM_EEPROM_WP
#define _PIN_63                                 (1 & 0x07) // Page 10-0xA6[2:0]
...
// VOLUME_ADJ
#define _PIN_102                                (7 & 0x07) // Page 10-0xAA[5:3]
...
// AUDIO_MUTE
#define _PIN_104                                (2 & 0x07) // Page 10-0xAC[3:1]
...
// DVI_HPD
#define _PIN_114                                (1 & 0x07) // Page 10-0xB2[4:2]
...
sbit bSWIIC_EDID_EEPROM_SCL_VGA                 = P3^0;
sbit bSWIIC_EDID_EEPROM_SDA_VGA                 = P3^1;

#define bSWIIC_EDID_EEPROM_SCL_DVI              (MCU_PORT73_PIN_REG_FFD2)//_PIN_121
#define bSWIIC_EDID_EEPROM_SDA_DVI              (MCU_PORT72_PIN_REG_FFD1)//_PIN_122
...
#define PCB_SW_IIC_SDA_SET()                    {\
                                                    ...
                                                    {                                          \
                                                        bSWIIC_EDID_EEPROM_SDA_DVI = _TRUE;         \
                                                    }                                          \
...
#define PCB_SW_IIC_SDA_CLR()                    {\
                                                    ...
                                                    {                                          \
                                                        bSWIIC_EDID_EEPROM_SDA_DVI = _FALSE;         \
                                                    }                                          \
                                                    else if(GET_SW_IIC_STATUS() == _IIC_FOR_DDC3) \
                                                    {                                          \
...
#define PCB_SW_IIC_SCL_SET()                    {\
                                                    ...
                                                    {                                          \
                                                        bSWIIC_EDID_EEPROM_SCL_DVI = _TRUE;         \
                                                    }                                          \
                                                    else if(GET_SW_IIC_STATUS() == _IIC_FOR_DDC3) \
                                                    {                                          \
...
#define PCB_SW_IIC_SCL_CLR()                    {\
                                                    ...
                                                    {                                          \
                                                        bSWIIC_EDID_EEPROM_SCL_DVI = _FALSE;         \
                                                    }                                          \
                                                    else if(GET_SW_IIC_STATUS() == _IIC_FOR_DDC3) \
                                                    {                                          \
...
sbit bFLASH_WRITE_PROTECT                             = P3 ^ 5; // Pin 112, P3.5 for Flash WP
...
#define bEEPROM_WRITE_PROTECT                   (MCU_PORTC3_PIN_REG_FFA0)     // Pin 63, PC.3
...
#define bEDID_WRITE_PROTECT                     (MCU_PORTC4_PIN_REG_FFA4)     // Pin 40, PC.4
...
#define bD0_CONNECT                             (MCU_PORTC0_PIN_REG_FFA3) // Pin 126, PC.0
#define bD1_CONNECT                             (MCU_EMPTY_PIN_ASSIGN_ADDRESS) // Pin 32 , PD.6
...
#define PCB_D1_PIN()                            (SET_D1_CABLE_STATUS(_FALSE))
...
sbit bD0_HPD                                    = P3 ^ 6; // Pin 113, P3.6 for HDMI
...
//sbit bD1_HPD                                    = P3 ^ 4; // Pin 111, P3.4 for DVI
sbit bD1_HPD                                    = P3 ^ 7; // Pin 114, P3.7 for DVI
...
// Macro of LED On/Off
//--------------------------------------------------
sbit bLED1                                      = P3 ^ 4; // Pin 110 , P3.3
sbit bLED2                                      = P3 ^ 3; // Pin 111 , P3.4
...
#define AD_KEY3                                 (MCU_ADC3_RESULT_FF0C)      
...
#define PCB_KEY_STATE(ucV0, ucV1, ucV2, ucV3, ucKeyState)   {\
                                                                ...
                                                                if((0 <= ucV1) && (ucV1 < 0x40))       \
                                                                    ucKeyState |= _POWER_KEY_MASK;     \
                                                                                                       \
                                                                if((0x60 <= ucV0) && (ucV0 < 0xb0))       \
                                                                    ucKeyState |= _RIGHT_KEY_MASK;      \
                                                                                                       \
                                                                if((0 <= ucV0) && (ucV0 < 0x40))    \
                                                                    ucKeyState |= _EXIT_KEY_MASK;      \
                                                                                                       \
                                                                if((0x50 <= ucV0) && (ucV0 < 0x70))    \
...
#define bBACKLIGHTPOWER                             (MCU_PORT81_PIN_REG_FFD7) // Pin 108, P8.1
...
#define bAudioEn                                (MCU_PORT80_PIN_REG_FFD6)//_PIN_105
...
//-----------------------------------------------
#define bMute                                   (MCU_PORT74_PIN_REG_FFD3)
...
//-----------------------------------------------
#define _AIN_AUDIO_VOL_FIXED                    0x0028//ym20130222 . max:0x003F
#define _DAC_AUDIO_VOL_FIXED                    0x0035//ym20130222 . max:0x003F
#define _VOLUME_MAX                             0xb4
...
#endif // End of #if(_PCB_TYPE == _PCB_WISTRON_715G5919_6192_AHP)
