...
void ScalerOsdDrawWindow(WORD usXStart, WORD usYStart, WORD usXEnd, WORD usYEnd)
{
    ...
    if((ScalerOsdFrameControlByteGet(0x016, _OSD_BYTE0) & ~(_BIT0) != _BIT0) && (GET_OSD_WINDOW_ROTATION_FUNCTION_ENABLE() == _ENABLE))
    {
        if((ScalerOsdFrameControlByteGet(0x016, _OSD_BYTE0) & _BIT4) == _BIT4)  // CW   _OSD_ROTATE_DEGREE_90
        {
            usXStart += (_OSD_POSITION_OFFSET_V - 1);
            usXEnd += (_OSD_POSITION_OFFSET_V - 1);
        }
        else                                                                    // CCW  _OSD_ROTATE_DEGREE_270
        {
            usYStart += (GET_OSD_POSITION_OFFSET_H() + GET_OSD_WINDOW_START_OFFSET());
            usYEnd += (GET_OSD_POSITION_OFFSET_H() + GET_OSD_WINDOW_START_OFFSET());
        }
    }
    else
    {
