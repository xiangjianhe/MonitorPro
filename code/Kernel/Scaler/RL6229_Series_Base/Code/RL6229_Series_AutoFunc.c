...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6229_Series_AutoFunc.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6229_SERIES_AUTOFUNC__
...
#if(_VGA_CUSTOM_AUTOCOLOR_PATTERN_SETTING == _ON)
#define _AUTO_CUSTOM_FRACTION_BIT                (3)
#define _AUTO_CUSTOM_CHECK_MARGIN                (2)

#define _AUTO_CUSTOM_WHITE_LEVEL                 ((WORD)253 << _AUTO_CUSTOM_FRACTION_BIT)
#define _AUTO_CUSTOM_WHITE_HSTART                (ScalerVgipGetCaptureHDelay() + ScalerVgipGetCaptureHStartPosition() + GET_INPUT_TIMING_HWIDTH() * 5 / 6)
#define _AUTO_CUSTOM_WHITE_HEND                  (_AUTO_CUSTOM_WHITE_HSTART + 0x0F)
#define _AUTO_CUSTOM_WHITE_VSTART                (GET_INPUT_TIMING_VHEIGHT() / 2)
#define _AUTO_CUSTOM_WHITE_VEND                  (_AUTO_CUSTOM_WHITE_VSTART + 0x0F)

#define _AUTO_CUSTOM_BLACK_LEVEL                 ((WORD)3 << _AUTO_CUSTOM_FRACTION_BIT)
#define _AUTO_CUSTOM_BLACK_HSTART                (ScalerVgipGetCaptureHDelay() + ScalerVgipGetCaptureHStartPosition() + GET_INPUT_TIMING_HWIDTH() / 6)
#define _AUTO_CUSTOM_BLACK_HEND                  (_AUTO_CUSTOM_BLACK_HSTART + 0x0F)
#define _AUTO_CUSTOM_BLACK_VSTART                (GET_INPUT_TIMING_VHEIGHT() / 2)
#define _AUTO_CUSTOM_BLACK_VEND                  (_AUTO_CUSTOM_BLACK_VSTART + 0x0F)

#define _AUTO_CUSTOM_CHECK_LEVEL                 ((WORD)51 << _AUTO_CUSTOM_FRACTION_BIT)
#define _AUTO_CUSTOM_CHECK_HSTART                (ScalerVgipGetCaptureHDelay() + ScalerVgipGetCaptureHStartPosition() + GET_INPUT_TIMING_HWIDTH() / 2)
#define _AUTO_CUSTOM_CHECK_HEND                  (_AUTO_CUSTOM_CHECK_HSTART + 0x0F)
#define _AUTO_CUSTOM_CHECK_VSTART                (GET_INPUT_TIMING_VHEIGHT() / 2)
#define _AUTO_CUSTOM_CHECK_VEND                  (_AUTO_CUSTOM_CHECK_VSTART + 0x0F)
#endif

#endif

#define GET_AUTO_ADC_GAIN_10BIT(value)           {\
                                                    switch(ScalerGetBit(CM_7A_AUTO_ADJ_CTRL0, (_BIT1 | _BIT0)))\
                                                    {\
                                                        default:\
                                                        case _BIT1:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C0_RED_GAIN_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BE_ADC_RGB_GAIN_LSB, (_BIT5 | _BIT4)) >> 4);\
                                                            break;\
                                                        case _BIT0:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C1_GRN_GAIN_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BE_ADC_RGB_GAIN_LSB, (_BIT3 | _BIT2)) >> 2);\
                                                            break;\
                                                        case 0:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C2_BLU_GAIN_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BE_ADC_RGB_GAIN_LSB, (_BIT1 | _BIT0)) >> 0);\
                                                            break;\
                                                    }\
                                                 }
#define SET_AUTO_ADC_GAIN_10BIT(value)           {\
                                                    switch(ScalerGetBit(CM_7A_AUTO_ADJ_CTRL0, (_BIT1 | _BIT0)))\
                                                    {\
                                                        default:\
                                                        case _BIT1:\
                                                            ScalerSetByte(P0_C0_RED_GAIN_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BE_ADC_RGB_GAIN_LSB, ~(_BIT5 | _BIT4), (((value) & 0x03) << 4));\
                                                            break;\
                                                        case _BIT0:\
                                                            ScalerSetByte(P0_C1_GRN_GAIN_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BE_ADC_RGB_GAIN_LSB, ~(_BIT3 | _BIT2), (((value) & 0x03) << 2));\
                                                            break;\
                                                        case 0:\
                                                            ScalerSetByte(P0_C2_BLU_GAIN_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BE_ADC_RGB_GAIN_LSB, ~(_BIT1 | _BIT0), (((value) & 0x03) << 0));\
                                                            break;\
                                                    }\
                                                 }

#define GET_AUTO_ADC_OFFSET_10BIT(value)         {\
                                                    switch(ScalerGetBit(CM_7A_AUTO_ADJ_CTRL0, (_BIT1 | _BIT0)))\
                                                    {\
                                                        default:\
                                                        case _BIT1:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C3_RED_OFFSET_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BF_ADC_RGB_OFFSET_LSB, (_BIT5 | _BIT4)) >> 4);\
                                                            break;\
                                                        case _BIT0:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C4_GRN_OFFSET_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BF_ADC_RGB_OFFSET_LSB, (_BIT3 | _BIT2)) >> 2);\
                                                            break;\
                                                        case 0:\
                                                            (value) = ((WORD)ScalerGetByte(P0_C5_BLU_OFFSET_MSG) << 2) |\
                                                                    (ScalerGetBit(P0_BF_ADC_RGB_OFFSET_LSB, (_BIT1 | _BIT0)) >> 0);\
                                                            break;\
                                                    }\
                                                 }
#define SET_AUTO_ADC_OFFSET_10BIT(value)         {\
                                                    switch(ScalerGetBit(CM_7A_AUTO_ADJ_CTRL0, (_BIT1 | _BIT0)))\
                                                    {\
                                                        default:\
                                                        case _BIT1:\
                                                            ScalerSetByte(P0_C3_RED_OFFSET_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BF_ADC_RGB_OFFSET_LSB, ~(_BIT5 | _BIT4), (((value) & 0x03) << 4));\
                                                            break;\
                                                        case _BIT0:\
                                                            ScalerSetByte(P0_C4_GRN_OFFSET_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BF_ADC_RGB_OFFSET_LSB, ~(_BIT3 | _BIT2), (((value) & 0x03) << 2));\
                                                            break;\
                                                        case 0:\
                                                            ScalerSetByte(P0_C5_BLU_OFFSET_MSG, ((value) >> 2));\
                                                            ScalerSetBit(P0_BF_ADC_RGB_OFFSET_LSB, ~(_BIT1 | _BIT0), (((value) & 0x03) << 0));\
                                                            break;\
                                                    }\
                                                 }
...
#if(_VGA_CUSTOM_AUTOCOLOR_SUPPORT == _ON)
EnumAutoResult ScalerAutoCustomUpdateAdcGainRGB(BYTE ucGainBias);
EnumAutoResult ScalerAutoCustomCompensateRGB(EnumAutoColorType enumAutoType);
EnumAutoResult ScalerAutoCustomUpdateAdcGain(WORD usPreciseTarget, BYTE ucColor);
EnumAutoResult ScalerAutoCustomCompensateChannel(BYTE ucColor);
EnumAutoResult ScalerAutoCompensatePreciseLevel(EnumADCGainOffset enumADCGainOffset, WORD usPreciseTarget);
#endif
...
#if(_VGA_ADC_SF_SHA_LPF_UNDER_25MHZ_SUPPORT == _ON)
    if((((DWORD)GET_INPUT_TIMING_HFREQ() * (DWORD)GET_INPUT_TIMING_HTOTAL()) / 10000) < 26)
    {
...
#endif
#if(_VGA_CUSTOM_AUTOCOLOR_SUPPORT == _ON)
        case _AUTP_TUNE_CUSTOM:
            pucFunctionUpdateGain = &ScalerAutoCustomUpdateAdcGainRGB;
            pucFunctionCompensate = &ScalerAutoCustomCompensateRGB;
            break;
#endif
        case _AUTO_TUNE_RGB:
...
void ScalerAutoInitializeAutoColor(EnumAutoColorType enumAutoType)
{
    ...
    if((enumAutoType != _AUTO_TUNE_RGB_GAIN_ONLY) && (enumAutoType != _AUTO_TUNE_YPBPR_GAIN_ONLY))
    {
        ...
        ScalerSetByte(P0_DC_TEST_PTN_GD, 0x00);
    ...
    ScalerSetByte(P0_EF_ABL_CTRL8, 0x00);
...
//--------------------------------------------------
// Description  : Search in all SHA gains(12~0) and choose the maximum available one
// Input Value  : none
// Output Value : none
//--------------------------------------------------
void ScalerAutoSearchShaGain(void)
...
EnumAutoResult ScalerAutoCustomUpdateAdcGainRGB(BYTE ucGainBias)
{
    ucGainBias = ucGainBias;

    if(ScalerAutoCustomUpdateAdcGain(_AUTO_CUSTOM_WHITE_LEVEL, _AUTO_RED_CHANNEL) == _AUTO_SUCCESS)
    {
        if(ScalerAutoCustomUpdateAdcGain(_AUTO_CUSTOM_WHITE_LEVEL, _AUTO_BLUE_CHANNEL) == _AUTO_SUCCESS)
        {
            if(ScalerAutoCustomUpdateAdcGain(_AUTO_CUSTOM_WHITE_LEVEL, _AUTO_GREEN_CHANNEL) == _AUTO_SUCCESS)
            {
                return _AUTO_SUCCESS;
            }
        }
...
EnumAutoResult ScalerAutoCustomCompensateRGB(EnumAutoColorType enumAutoType)
{
    ...
    if(ScalerAutoCustomCompensateChannel(_AUTO_RED_CHANNEL) == _AUTO_SUCCESS)
    {
        if(ScalerAutoCustomCompensateChannel(_AUTO_GREEN_CHANNEL) == _AUTO_SUCCESS)
        {
            if(ScalerAutoCustomCompensateChannel(_AUTO_BLUE_CHANNEL) == _AUTO_SUCCESS)
            {
                return _AUTO_SUCCESS;
            }
        }
...
//--------------------------------------------------
// Description  : Measure current maximum and update ADC gain
// Input Value  : usPreciseTarget:     target maximum value
//                ucColor:      ADC channel to be tuned
// Output Value : _AUTO_SUCCESS or _AUTO_FAIL
//--------------------------------------------------
EnumAutoResult ScalerAutoCustomUpdateAdcGain(WORD usPreciseTarget, BYTE ucColor)
{
    WORD usPreciseGain = 0;
    WORD usPreciseLevel = 0;

    SET_AUTO_ADC_CHANNEL(ucColor);
    GET_AUTO_ADC_GAIN_10BIT(usPreciseGain);

    // Stop image auto-function measure
    ScalerSetBit(CM_7D_AUTO_ADJ_CTRL1, ~_BIT0, 0x00);

    // Disable field select; Enable LPF filter; Disable hardware acceleration;
    ScalerSetByte(CM_79_DIFF_THRESHOLD, 0x00);
    ScalerSetBit(CM_7A_AUTO_ADJ_CTRL0, ~(_BIT7 | _BIT4), 0x00);

    // Set measure boundary
    ScalerMeasureAutoFuncHBoundInitial(_AUTO_CUSTOM_WHITE_HSTART, _AUTO_CUSTOM_WHITE_HEND);
    ScalerMeasureAutoFuncVBoundInitial(_AUTO_CUSTOM_WHITE_VSTART, _AUTO_CUSTOM_WHITE_VEND);
    ...
    usPreciseLevel = (GET_AUTO_RESULT_DWORD() << _AUTO_CUSTOM_FRACTION_BIT) /
                     (_AUTO_CUSTOM_WHITE_VEND - _AUTO_CUSTOM_WHITE_VSTART + 1) /
                     (_AUTO_CUSTOM_WHITE_HEND - _AUTO_CUSTOM_WHITE_HSTART + 1);

    if(usPreciseLevel < usPreciseTarget)
    {
        PDATA_WORD(0) = (WORD)(((DWORD)(usPreciseTarget - usPreciseLevel) * ((WORD)usPreciseGain + 256)) / usPreciseTarget);
        usPreciseGain -= ((PDATA_WORD(0) == 0) ? 1 : PDATA_WORD(0));
    }
    else
    {
        PDATA_WORD(0) = ((usPreciseLevel - usPreciseTarget) >> _AUTO_CUSTOM_FRACTION_BIT);
        usPreciseGain += ((PDATA_WORD(0) == 0) ? 1 : PDATA_WORD(0));
    }

    SET_AUTO_ADC_GAIN_10BIT(usPreciseGain);
...
EnumAutoResult ScalerAutoCustomCompensateChannel(BYTE ucColor)
{
    ...
    while(--ucLoop)
    {
        // Compensate black level using offset
        ScalerMeasureAutoFuncHBoundInitial(_AUTO_CUSTOM_BLACK_HSTART, _AUTO_CUSTOM_BLACK_HEND);
        ScalerMeasureAutoFuncVBoundInitial(_AUTO_CUSTOM_BLACK_VSTART, _AUTO_CUSTOM_BLACK_VEND);
        if(ScalerAutoCompensatePreciseLevel(_ADC_OFFSET, _AUTO_CUSTOM_BLACK_LEVEL) == _AUTO_FAIL)
        {
            return _AUTO_FAIL;
        }

        // Compensate white level using gain
        ScalerMeasureAutoFuncHBoundInitial(_AUTO_CUSTOM_WHITE_HSTART, _AUTO_CUSTOM_WHITE_HEND);
        ScalerMeasureAutoFuncVBoundInitial(_AUTO_CUSTOM_WHITE_VSTART, _AUTO_CUSTOM_WHITE_VEND);
        if(ScalerAutoCompensatePreciseLevel(_ADC_GAIN, _AUTO_CUSTOM_WHITE_LEVEL) == _AUTO_FAIL)
        {
            return _AUTO_FAIL;
        }

        // Compensate check level using offset
        ScalerMeasureAutoFuncHBoundInitial(_AUTO_CUSTOM_CHECK_HSTART, _AUTO_CUSTOM_CHECK_HEND);
        ScalerMeasureAutoFuncVBoundInitial(_AUTO_CUSTOM_CHECK_VSTART, _AUTO_CUSTOM_CHECK_VEND);
        if(ScalerAutoCompensatePreciseLevel(_ADC_OFFSET, _AUTO_CUSTOM_CHECK_LEVEL) == _AUTO_FAIL)
        {
            return _AUTO_FAIL;
        }
    }
...
//--------------------------------------------------
// Description  : Color compensation for precise target (10-bit)
// Input Value  : enumADCGainOffset -> adjust gain or offset
//                usPreciseTarget -> target level
// Output Value : _AUTO_SUCCESS or _AUTO_FAIL
//--------------------------------------------------
EnumAutoResult ScalerAutoCompensatePreciseLevel(EnumADCGainOffset enumADCGainOffset, WORD usPreciseTarget)
{
    BYTE ucLoop = 10;
    BYTE ucFrameCount = 0;
    WORD usPreviousDiff = 0x0FFF;
    WORD usPreciseLevel = 0;
    WORD usPreciseValue = 0;
    WORD usHwidth = (((WORD)ScalerGetBit(CM_70_H_BOUNDARY_H, 0x0F) << 8) | ScalerGetByte(CM_72_H_BOUNDARY_END_L)) -
                    (((WORD)ScalerGetBit(CM_70_H_BOUNDARY_H, 0xF0) << 4) | ScalerGetByte(CM_71_H_BOUNDARY_STA_L)) + 1;
    WORD usVheight = (((WORD)ScalerGetBit(CM_73_V_BOUNDARY_H, 0x0F) << 8) | ScalerGetByte(CM_75_V_BOUNDARY_END_L)) -
                     (((WORD)ScalerGetBit(CM_73_V_BOUNDARY_H, 0xF0) << 4) | ScalerGetByte(CM_74_V_BOUNDARY_STA_L)) + 1;
    ...
    while(--ucLoop)
    {
        ...
        for(ucFrameCount = 0; ucFrameCount < 4; ++ucFrameCount)
        {
            // Normal boundary; Software mode; Measure maximum / minimum; Compare mode; Auto-balance; Start measure;
            ScalerSetBit(CM_7D_AUTO_ADJ_CTRL1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT1 | _BIT0), (_BIT5 | _BIT4 | _BIT0));
            ...
            usPreciseLevel += (GET_AUTO_RESULT_DWORD() << _AUTO_CUSTOM_FRACTION_BIT) / usHwidth / usVheight;
        }

        usPreciseLevel >>= 2;

        if(abs(usPreciseLevel - usPreciseTarget) < _AUTO_CUSTOM_CHECK_MARGIN)
        {
            return _AUTO_SUCCESS;
        }
        else
        {
            if(enumADCGainOffset == _ADC_GAIN)
            {
                GET_AUTO_ADC_GAIN_10BIT(usPreciseValue);
                (usPreciseLevel > usPreciseTarget) ? (++usPreciseValue) : (--usPreciseValue);
                SET_AUTO_ADC_GAIN_10BIT(usPreciseValue);
            }
            else
            {
                GET_AUTO_ADC_OFFSET_10BIT(usPreciseValue);
                (usPreciseLevel > usPreciseTarget) ? (++usPreciseValue) : (--usPreciseValue);
                SET_AUTO_ADC_OFFSET_10BIT(usPreciseValue);
            }

            // Skip compensation if non-homogeneity occurs
            PDATA_WORD(0) = abs(usPreciseLevel - usPreciseTarget);
            if(((DWORD)usPreviousDiff + _AUTO_CUSTOM_CHECK_MARGIN) < PDATA_WORD(0))
            {
                break;
            }

            usPreviousDiff = PDATA_WORD(0);
        }
    }

    return _AUTO_SUCCESS;
}
#endif // End of #if(_VGA_CUSTOM_AUTOCOLOR_SUPPORT == _ON)
