...
//----------------------------------------------------------------------------------------------------
// ID Code      : UserEeprom.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RTD_EEPROM_OSD__
...
//--------------------------------------------------
#define _BRIGHTNESS                             	0x80
#define _CONTRAST									_OSD_CONTRAST_DEF_VALUE//lorimiao 0717
#define _DEFAULT_BACKLIGHT							_OSD_BRIGHTNESS_DEF_VALUE

//--------------------------------------------------
// Six Color Data Default Values
//--------------------------------------------------
#define _SIX_COLOR_HUE_R                            50
#define _SIX_COLOR_SATURATION_R                     50

#define _SIX_COLOR_HUE_Y                            50
#define _SIX_COLOR_SATURATION_Y                     50

#define _SIX_COLOR_HUE_G                            50
#define _SIX_COLOR_SATURATION_G                     50

#define _SIX_COLOR_HUE_C                            50
#define _SIX_COLOR_SATURATION_C                     50

#define _SIX_COLOR_HUE_B                            50
#define _SIX_COLOR_SATURATION_B                     50

#define _SIX_COLOR_HUE_M                            50
#define _SIX_COLOR_SATURATION_M                     50
...
{
    _DEFAULT_BACKLIGHT,		// ucBackLight;           
    ...
    10,                     // ucOsdTimeout;
//--------------------------------------------------------
    _LANGUAGE_DEFAULT,               // b4Language;
    _COLOREFFECT_STANDARD,    // b4ColorEffect;
//--------------------------------------------------------
    _COLOR_TEMP_DEF,        // b4ColorTempType;
...
//--------------------------------------------------------
    ...
    _ON,                    // b1DDCCIStatus;
    _OFF,                   // b1OsdDCRStatus;   
    _OD_STATUS_DEFAULT,//_ON,// bODStatus;
//--------------------------------------------------------
	_OD_NORMAL_GAIN,		// ucODGain;  
    _DEFAULT_HUE,           // cHue;
...
//--------------------------------------------------------
	...
	0,						// b33DModeStatus : 3;
//--------------------------------------------------------
	0, 						// 3NovoVisionStatus : 3;        //Eli Add
	1, 						// b2OsdButtonRepeatStatus : 2;        //Eli Add
	0,						// b1KeypadBuzzerStatus : 1;      //Eli Add        
//--------------------------------------------------------	
    50,                     // ucVolume;
    0x00,					// b33DConvergenceMode : 3;
    _OFF,                   // b1OSDLockFlag : 1;//lorimiao 20130729 for OSD lock issue
	_ON,					// b1FactoryReset : 1;
	1,                      // b1DPType : 1 (DP1.1=0,DP1.2=1)
    _OFF,                   // b1LowBlueLightStatus : 1;	
	_COLOR_TEMP_DEF,		//b4PresetColorTempBackUp
#if(_ENERGY_STAR_SUPPORT == _ON)
	_ON,				   // b1OsdESStatus : 1; 
#else
	_OFF,					// b1OsdESStatus : 1; 
#endif
};
...
{
    {
        _CT7200_LenovoPrj_RED,
        _CT7200_LenovoPrj_GREEN,
        _CT7200_LenovoPrj_BLUE,
    },
    
    {
        _CTSRGB_LenovoPrj_RED,
        _CTSRGB_LenovoPrj_GREEN,
        _CTSRGB_LenovoPrj_BLUE,
    },

    {
        _CT6500_LenovoPrj_RED,
        _CT6500_LenovoPrj_GREEN,
        _CT6500_LenovoPrj_BLUE,
    },

    {
        _CT9300_LenovoPrj_RED,
        _CT9300_LenovoPrj_GREEN,
        _CT9300_LenovoPrj_BLUE,
    },

    {
        _CTUSER_LenovoPrj_RED,
        _CTUSER_LenovoPrj_GREEN,
        _CTUSER_LenovoPrj_BLUE,
    },
#if(_LOW_BLUE_LIGHT_FUNCTION == _ON)
    {
        _CTLOWBLUELIGHT_RED,
        _CTLOWBLUELIGHT_GREEN,
        _CTLOWBLUELIGHT_BLUE,
    },
#endif    
};
...
void RTDEepromSaveBriCon(void);
void RTDEepromLoadBriCon(void);
...
void RTDFlashSaveBriCon(void);
void RTDFlashLoadBriCon(void);
...
void RTDEepromStartup(void)
{
    ...
    if(pData[0] != _CHECKSUM)
    {
	    ...
	    NVRamRestoreFactoryDate();
        FactoryEdidCheckVGA24C02();
#endif            
...
#if (_FACTORY_MODE_OWNER == _OWNER_TPV)//DFM
	TPVNVRamRestoreFactoryDate();   //工廠DFM相關Flag初始化
#elif(_FACTORY_MODE_OWNER == _OWNER_FOXCONN)//lorimiao 0502
	NVRamRestoreFactoryDate();
	NVRamRestoreFacoryBaseData();
#elif(_FACTORY_MODE_OWNER == _OWNER_WISTRON)
    NVRamRestoreFactoryData();
    FactoryEdidCheckFirstPortThreeByte();
#endif
...
void RTDEepromLoadOSDData(void)
{
    ...
    UserInterfaceCheckNVRamData(_CHECK_OSD_DATA_LOAD); 
#endif
}
...
void RTDEepromLoadBriCon(void)
{
    UserCommonEepromRead(_BRICON_DATA_ADDRESS +  sizeof(StructBriConDataType), sizeof(StructBriConDataType), (BYTE *)(&g_stBriConData));
#if(_FACTORY_MODE_OWNER == _OWNER_TPV)	
	UserInterfaceCheckNVRamData(_CHECK_BRICON_DATA_LOAD);
#endif
}
...
void RTDEepromSaveBriCon(void)
{
    UserCommonEepromWrite(_BRICON_DATA_ADDRESS +  sizeof(StructBriConDataType), sizeof(StructBriConDataType), (BYTE *)(&g_stBriConData));
}
...
void RTDEepromRestoreBriCon(void)
{
    g_stBriConData = g_stBriConDefaultData;
    RTDEepromSaveBriCon();
}
...
#if(_FACTORY_MODE_OWNER == _OWNER_TPV)		
    UserInterfaceCheckNVRamData(_CHECK_COLORTEMP_DATA_LOAD|(ucColorTempType<<4));
#endif
}
...
#if(_FACTORY_MODE_OWNER == _OWNER_WISTRON)
		    NVRamRestoreFactoryData();
            FactoryEdidCheckFirstPortThreeByte();//ym20141013 . for EDID Write default after ISP
#endif
        }
        else
        {
...
void RTDFlashStartup(void)
{
    ...
    if((ucChecksum != _CHECKSUM) || (ucVersionCode != _VERSION_CODE))
    {
        ...
        g_stColorProcData = g_stColorTempDefaultData[_CT_USER];
        if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_COLORPROC0_7200 + _CT_USER, pData, 1) == _TRUE)
        {
...
void RTDFlashLoadBriCon(void)
{
    if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_BRICON , pData, sizeof(StructBriConDataType)) == _TRUE)
    {
...
void RTDFlashSaveBriCon(void)
{
    UserInterfaceFlashSaveItem(_FLASH_ITEMID_BRICON , (BYTE *)(&g_stBriConData), sizeof(StructBriConDataType));
}
...
void RTDFlashRestoreBriCon(void)
{
    ...
    if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_BRICON , pData, 1) == _TRUE)
    {
        RTDFlashSaveBriCon();
    }
}
...
void RTDFlashLoadColorSetting(BYTE ucColorTempType)
{
    if(ucColorTempType != _CT_OFF)
    {
        ...
        {
            if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_COLORPROC0_7200 + ucColorTempType, pData, sizeof(StructColorProcDataType)) == _TRUE)
            {
...
void RTDFlashSaveColorSetting(BYTE ucColorTempType)
{
    if(ucColorTempType != _CT_OFF)
    {
        UserInterfaceFlashSaveItem(_FLASH_ITEMID_COLORPROC0_7200 + ucColorTempType, (BYTE*)(&g_stColorProcData), sizeof(StructColorProcDataType));
    }
}
...
void RTDFlashRestoreColorSetting(void)
{
    ...
    for(ucColorTempIndex = 0; ucColorTempIndex < _CT_COLORTEMP_AMOUNT; ucColorTempIndex++)
    {
        ...
        if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_COLORPROC0_7200 + ucColorTempIndex, pData, 1) == _TRUE)
        {
...
void RTDFlashRestoreUserColorSetting(void)
{
    ...
    if(UserInterfaceFlashLoadItem(_FLASH_ITEMID_COLORPROC4_USER, pData, 1) == _TRUE)
    {
